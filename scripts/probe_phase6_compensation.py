...
def test_the_m10_compensation_probe_behaves_as_specified():
    """### PYTEST-COLLECTED ENTRY POINT (CLAUDE.md §6). U8.4 reconciled this probe's freeze-guard set
    (M1/M9 left it, rule 20) and its M1/M9 headline; the standard runner OPERATES the whole probe here —
    `python -m pytest scripts/probe_phase6_compensation.py` — and reads its exit status, not only the
    `__main__` CLI. `main([])` runs every case in canonical order and returns 0 only on "behaviours as
    specified, 0 wrong". Deliberately NOT swept by `pytest eval` (outside `testpaths`, `probe_*.py`)."""
    assert main([]) == 0, (
        "the M10 compensation probe reported behaviours NOT as specified (see its '... wrong' line); "
        "the U8.4 reconciliation of its freeze set is not observed green here.")
