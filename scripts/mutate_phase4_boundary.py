...
TEXT_CASES = [
    ...
    ("B39 U4.11/EP-1: a SECOND live write driver is constructed outside the single named agent "
     "factory (an unaccounted effect route beside the residual)",
     EP1,
     "    lock = BrowserLock(lock_path) if lock_path else None\n\n"
     "    def _resolve(load_ref: str)",
     "    lock = BrowserLock(lock_path) if lock_path else None\n"
     "    _live = CdpActuator(CdpBrowserSession(cdp_url=cdp_url))  # MUTANT: second effect route\n\n"
     "    def _resolve(load_ref: str)",
     f"{IMPORTS}::test_the_callback_servers_only_actuator_use_is_the_write_path"),
]
