...
def test_the_governed_route_executes_the_full_order_and_verifies(tmp_path):
    """Work Item / Pipeline identity -> approval -> checkpoint -> witness -> grant -> claim ->
    bounded op -> adapter -> readback -> VERIFIED -> closure, with exactly one external attempt."""
    op = _op()
    store, kernel, clock, handle, fp = _mint(tmp_path, op)

    # the recorded human decision advances the pipeline (no actuator touched here)
    ...
    assert _security(store, "GovernedWriteIntentQueued"), "the pipeline did not advance"
