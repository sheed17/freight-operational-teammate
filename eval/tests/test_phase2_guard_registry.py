...
GUARD_REGISTRY: dict[str, tuple[str, str]] = {
    ...
    "test_integration_topology.py": (RETAIN, "R-21/R-21b integration topology: the first-parent chain must bind to the certified pair and the status-metadata commit must have exactly one parent (the content commit MAY be a merge - that is how a diverged main is integrated), the fast-forward-only procedure rehearsed hermetically every suite AT THE FINALIZER-TIME STATE, and the obligation kept recorded in the procedure document, the risk register and the progress protocol"),
    ...
    "test_phase6_pipeline_instance.py": (RETAIN, "P6's second entity capability held to the two claims it makes: that a broker cannot be billed twice for one load, and that Neyma never turns 'we do not know whether it happened' into 'it failed'. It asserts AC-MACH-000's bijection between the implementation's declarative transition table and the twenty-five rows of state-machines/02-pipeline-instance.machine.md sec 14 by EXACT SET EQUALITY of transition identifiers, with a positive control performing a same-count substitution so the oracle is demonstrated to be a set comparison and not a count, and with the denominator read from foundational-machine-acceptance.md's own coverage table rather than typed; it drives the exhaustive (state x trigger) sweep over a PROVEN population of 400 pairs, 23 legal and 377 illegal, after separately proving every one of the sixteen states reachable, and requires every illegal pair to persist nothing while recording IllegalTransitionAttempted to the audit backbone AND security_events with a DISTINCT record per distinct attempt; it proves the Layer-1 reservation is a database constraint rather than a read-then-write, that its predicate is exactly the terminal set so that NEEDS_VERIFICATION holds the key and blocks a retry, and that a duplicate proposal is absorbed once per distinct proposal reference; it proves GRANTED is unreachable without BOTH a real checkpoint witness and a real grant at the DATABASE, that the witness binding is write-once, that terminal is absolutely final, that FAILED is unwritable without affirmative proof at the guard AND at the database, and that no timer moves NEEDS_VERIFICATION; it proves the PL-8 and PL-9 co-commits by FAULT INJECTION - handing the machine a stale row snapshot so the row write fails after the kernel succeeded - and asserts the witness, the grant and the claim roll back with it, which is the only shape in which 'never both, never neither' is observable; and it proves the capability ships dark by walking the whole transitive import closure, by an AST scan requiring zero production importers, and by measured zero witnesses and zero grants across every pre-checkpoint state"),
}
