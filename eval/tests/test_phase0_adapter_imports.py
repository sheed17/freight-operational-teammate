...
def test_the_callback_server_constructs_no_live_write_driver():  # EP-1 write CUT (this session)
    ...
    # Population proof (false-green defense): the AST walk found real name-calls, so the negative
    # assertions below are not vacuously green over an empty parse. run_callback_server is the one
    # call this entry point exists to make — its absence means the parse is degenerate.
    assert "run_callback_server" in constructed, (
        "the callback server no longer calls run_callback_server — the AST parse is degenerate and "
        "the no-live-write-driver checks below would be vacuous")
    ...
    # Population proof: the import walk found real modules (this file imports action_callback), so
    # the forbidden-adapter-import check below is not vacuous over an empty parse.
    assert "action_callback" in imported, (
        "the import walk is degenerate — the forbidden-adapter-import check below would be vacuous")
