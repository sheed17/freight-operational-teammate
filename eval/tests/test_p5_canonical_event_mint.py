...
def test_the_recorded_program_invariants_are_untouched_by_this_unit():
    ...
    # ### F-04 RESTORED (targeted review of the rejected closure candidate 5e9480b). This read
    # `for cp in p5.get("landed_checkpoints", []) or []:` — vacuous on an empty list, and mutation
    # M4 proved an emptied population was MISSED by all six guards that read the field. The
    # population assertion is the canonical guard contract (CLAUDE.md §9: a negative assertion needs
    # a proven population), and it was present at the parent commit; only the move out of the
    # else-branch dropped it.
