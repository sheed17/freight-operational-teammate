...
def test_the_write_module_mints_no_authority():
    ...
    # POPULATION PROOF. Every claim below is a negative over a parsed corpus, so
    # a parse that silently yielded nothing would leave this test green while
    # proving nothing - and what it exists to prove is containment. tms_write is
    # the module's one documented intra-adapter composition edge: if that is
    # missing, the parse is degenerate, not the adapter innocent.
    assert "tms_write" in imported, (
        "browser_use_write does not import tms_write - the parse is degenerate, so the "
        "containment assertions below would be vacuously true"
    )
    ...
    assert "SandboxInvoiceWriteAdapter" in source, (
