...
def _prose_blocks(text: str, table_lines: set[int]):
    """Contiguous non-blank prose. A table line always breaks the run, so a row can never merge
    with the prose around it - nor prose with a row."""
    lines = text.split("\n")
    buf: list[str] = []
    buf_off = off = 0
    for i, line in enumerate(lines):
...
def parse_status_claims(text: str, risk_id: str = RISK_ID) -> list[StatusClaim]:
    ...
    for unit in claim_units(text):
        ...
        if block is not None and block.is_historical:
            ...
            # the marker must GOVERN the claim - see the module docstring (R-03)
