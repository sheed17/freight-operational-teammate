...
def verify_governed_approval(
    ...
    *,
    ...
    # approval-identity-bound. The payload hash deliberately does NOT cover `approval_id` (it binds
    # the approved FACTS, and two approvals of the same facts must hash alike), so without this
    # check a token signed for a DIFFERENT approval of the same operation verified — and the grant
    # would then be authorized by an approval identity the presented operation never named. That is
    # the F-01 defect in miniature, and it is refused here.
    if not _eq(approval.approval_id, op.approval_id):
        raise BindingMismatch(
            f"approval-id binding mismatch: envelope={approval.approval_id!r} "
            f"op={op.approval_id!r} — a grant may not be authorized by a different approval than "
            f"the one bound to this operation")
