...
from .migrations.phase6_compensations import (
    P6CM_EXEMPT_TABLES,
    P6CM_INDEXES,
    P6CM_REPLACED_INDEXES,
    P6CM_TARGET_SCHEMA,
    P6CM_TENANT_TABLES,
    create_phase6_compensations_schema,
    phase6_compensations_readiness_problems,
    stamp_phase6_compensations_version,
)
from .migrations.phase6_conflicts import (
    P6CF_EXEMPT_TABLES,
    P6CF_INDEXES,
    P6CF_REPLACED_INDEXES,
    P6CF_TARGET_SCHEMA,
    P6CF_TENANT_TABLES,
...
_ALL_TARGET_SCHEMA: dict[str, str] = {
    ...
    **P6IBC_TARGET_SCHEMA,
    # M7's Conflict: two new tables. `conflicts` holds FKs into tenant_humans (M1, the named owner and
    # the decision-human) and IS the durable field condition; `conflict_parties` holds a self-FK into
    # conflicts, an FK into identity_binding_claims (M6, a claim party) and one into observations (M5, an
    # observation party). Merged after M6, M5 and M1.
    **P6CF_TARGET_SCHEMA,
    # M8's Expectation: two new tables. `expectations` holds FKs into tenant_humans (M1, the named
    # OVERDUE/INDETERMINATE owner), observations (M5, the discharging and subject observations) and
    # observation_coverage (M8's own health record, single and composite — the composite ties
    ...
    **P6XC_TARGET_SCHEMA,
    # M10's Compensation: one new table. `compensations` holds FKs into tenant_humans (M1, the named
    # owner), effect_grants (M3, the VERIFIED original effect being undone AND the K-4 provenance of the
...
def create_canonical_schema(conn: sqlite3.Connection) -> None:
    ...
    # P6's M5, the Observation. Built AFTER the Work Item because its `owner_id` holds a foreign key
    # into `tenant_humans` (M1) — the named human who owns an UNBOUND/UNPARSEABLE exception. On a
    ...
    # P6's M7, the Conflict. Built LAST of the P6 units because `conflict_parties` holds FKs into
    # identity_binding_claims (M6) and observations (M5), and `conflicts` holds FKs into tenant_humans
    # (M1). On a fresh database the merged DDL already built the shape; on a migrated one this creates
    # the two tables. Marker-last, like every other phase.
    create_phase6_conflicts_schema(conn, now=_now())
    if not phase6_conflicts_readiness_problems(conn):
        stamp_phase6_conflicts_version(conn, now=_now())
    # P6's M8, the Expectation. Built LAST of the P6 units because `expectations` holds FKs into
    # tenant_humans (M1, the OVERDUE/INDETERMINATE owner), observations (M5, the discharging and
    # subject observations) and its own observation_coverage table (the composite FK that ties
    # coverage_health to the real coverage row). On a fresh database the merged DDL already built the
    # shape; on a migrated one this creates the two tables. Marker-last, like every phase.
    ...
    # P6's M10, the Compensation. Built LAST of the P6 units because `compensations` holds FKs into
    # tenant_humans (M1), effect_grants (M3, the VERIFIED original effect), pipeline_instances (M2, the
    # executing attempt) and approvals (M4, the human authorisation). On a fresh database the merged DDL
    # already built the shape; on a migrated one this creates the table. Marker-last, like every phase.
