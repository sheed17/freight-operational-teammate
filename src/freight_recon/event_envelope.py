...
# `trg_event_outbox_strict_version_owner` trigger that enforces it, because a rule and its
# enforcement living in two files is how "strict per-aggregate ordering" quietly becomes a comment.
# A trigger and not a UNIQUE index: EF-2 legitimately emits GrantClaimed AND EffectAttempted on one
# effect_grant at one version, so uniqueness over (tenant, aggregate_type, aggregate_id,
# aggregate_version) would make the canonical claim path uninsertable. The invariant that actually
# holds is that no two DIFFERENT producer transitions may own one version, which no index expresses.
