...
#: The default port for each scheme this surface will fetch. A URL that states its scheme's default
#: port explicitly (`https://tms.test:443/x`) is the SAME origin as one that omits it, so the port is
#: normalized before comparison rather than compared as written.
_DEFAULT_PORTS: dict[str, int] = {"http": 80, "https": 443}

#: A syntactically plausible hostname. Deliberately strict: labels of unreserved characters
#: separated by single dots. This refuses embedded whitespace, empty labels (`tms..test`), a
#: trailing-dot-only host, and the assorted malformed hosts a parser might otherwise accept.
_HOSTNAME_RE = re.compile(r"^(?!-)[A-Za-z0-9-]{1,63}(?<!-)(\.(?!-)[A-Za-z0-9-]{1,63}(?<!-))*$")


@dataclass(frozen=True, slots=True)
class Origin:
    """A NORMALIZED web origin: scheme, hostname, effective port. Compared as three parsed
    components, never as a substring of a URL — which is the whole point of F-02."""
...
def parse_origin(url: object) -> Origin | None:
    """The normalized `Origin` of `url`, or None when it has none this surface may trust.

    Returns None — never a guess — for every shape the origin policy must refuse:

      * an empty, unparseable or relative reference (it has no origin of its own);
      * a scheme outside `http`/`https` (`javascript:`, `data:`, `file:`, and anything unforeseen);
      * a URL carrying USERINFO (`https://trusted.example@evil.example/`), because the authority
        that matters is the host AFTER the `@` and a reader's eye stops before it — this surface
        refuses the shape outright rather than relying on everyone parsing it correctly;
      * a malformed or empty host, or a port that is not a valid integer in range.

    The effective port is filled in from the scheme when the URL omits it, so `https://tms.test` and
    `https://tms.test:443` are one origin and `https://tms.test:8443` is a different one.
    """
    u = str(url or "").strip()
    if not u:
        return None
    ...
    try:
        ...
        return None
    return Origin(scheme=scheme, host=host, port=int(port if port is not None else _DEFAULT_PORTS[scheme]))


def _origin_defect(url: object) -> str:
    """WHY `url` has no trustworthy origin, named specifically. A refusal reason that lists three
    possible causes cannot be audited, and F-02's item 7 is precisely that a reason must describe
    the check that actually ran."""
    u = str(url or "").strip()
    if not u:
        return "is empty"
    try:
    ...
    if parts.username is not None or parts.password is not None or "@" in (parts.netloc or ""):
        return ("carries embedded credentials (userinfo before '@'), so the host a reader sees is "
                "not the host that would be fetched")
    scheme = (parts.scheme or "").lower()
    if not scheme:
        return "states no scheme, so it has no origin of its own"
    if scheme not in _DEFAULT_PORTS:
        return f"uses scheme {scheme!r}, which is not an http(s) document fetch"
    try:
        host = (parts.hostname or "").lower()
    except ValueError:
        return "has an unparseable host"
    if not host:
        return "states no host"
    if not _HOSTNAME_RE.match(host):
        return f"has a malformed host {host[:40]!r}"
    try:
        parts.port
    except ValueError:
        return "states a port that is not a valid integer in range"
    return "has no usable origin"


def _is_relative_reference(url: str) -> bool:
    """Is this a genuinely relative reference — one with NO host component of its own?

    `//evil.example/path` is deliberately excluded: it is SCHEME-relative, it names a different
    host, and treating a leading `/` as "relative, therefore same-origin" is exactly the F-02
    defect. Only a single leading `/`, or a query/fragment reference, is relative here.
    """
    u = str(url or "").strip()
    if not u:
        return False
...
def embedded_foreign_url_reason(url: str, established_origin: str | None) -> str:
    """"" when no query parameter of `url` smuggles a FOREIGN absolute URL, else the refusal reason.

    A load-detail document has no legitimate need to carry an absolute URL in its query string, and
    `?next=https://evil.example/` is the ordinary shape of an open-redirect handoff. The navigation
    target itself is same-origin, so the origin check above cannot see this; it is refused here as
    an independent barrier. A parameter naming the ESTABLISHED origin is left alone — the refusal is
    for FOREIGN targets, not for absolute URLs as such.
    """
    established = parse_origin(established_origin) if established_origin else None
    try:
        query = urlsplit(str(url or "")).query
    except ValueError:
        return f"unparseable query in {str(url)[:60]!r}"
    for key, value in parse_qsl(query, keep_blank_values=True):
        raw = str(value or "").strip()
        if not raw:
            continue
        if raw.lower().startswith(_REFUSED_SCHEMES):
            return (f"query parameter {key!r} carries a refused scheme — a code or local-resource "
                    "target smuggled through a parameter")
        candidate = parse_origin(raw)
        if candidate is None and raw.startswith("//"):
            # scheme-relative inside a parameter: no scheme to parse, still names a foreign host
            candidate = parse_origin("https:" + raw)
        if candidate is not None and (established is None or candidate != established):
            return (f"query parameter {key!r} carries an absolute URL to {candidate.as_string()}, "
                    "which is not the established origin — an encoded off-origin redirect")
    return ""
...
def link_origin_refusal(
    href: str,
    resolved: str,
    *,
    url_filter: str | None,
    established_origin: str | None,
) -> str:
    """"" when a page-published anchor may be navigated, else the reason it is refused (F-02).

    THE WHOLE ORIGIN DECISION FOR A SELECTED LINK, in one place. It covers both URLs a page can
    present — the raw `href` attribute and the URL the browser actually resolved (which a `<base>`
    tag can make different) — and both ways a foreign target can travel: as the navigation target
    itself, and as an absolute URL smuggled into a query parameter.

    This is the check whose absence the independent hostile review found: `select_load_detail_link`
    performed no origin test at all, so a cross-origin href with the right visible text was
    selectable as "provenance-bound", and the allow-reason claimed a TMS domain allowlist that had
    never run.
    """
    for label, candidate in (("anchor href", href), ("the browser-resolved URL", resolved)):
        target = str(candidate or "").strip()
        if not target:
            continue
        ok, reason = navigation_target_is_allowed(
            target, url_filter, established_origin=established_origin)
        if not ok:
            return f"{label} {target[:60]!r} refused by the origin policy: {reason}"
        smuggled = embedded_foreign_url_reason(target, established_origin)
        if smuggled:
            return f"{label} {target[:60]!r} refused: {smuggled}"
    return ""
...
def select_load_detail_link(
    ...
    *,
    ...
    sibling row, a lookalike identifier, a `data-method` anchor, a redirect-shaped route, a
    cross-origin href) is unit-testable with no browser at all. Ambiguity is a REFUSAL, never a
    pick — if the page offers two different documents for one load, this surface does not get to
    guess which one is safe.

    ORIGIN IS CHECKED HERE, INDEPENDENTLY (F-02). `established_origin` is a REQUIRED keyword and
    fails closed when absent: a page-published link is never trusted to stay on the TMS origin just
    because it is otherwise well-shaped. Both the raw attribute AND the browser-resolved URL are
    origin-checked, so a `<base>` tag cannot resolve a benign attribute to a foreign host. This runs
    IN ADDITION to — never instead of — the destructive-route, method-override, unsafe-scheme,
    anchor-shape and identity-binding barriers below.
    """
    lid = _norm(load_id)
    if not lid:
    ...
    for link in list(row.get("links") or []):
        ...
        if not ok:
            last_reason = reason
            continue
        # ORIGIN POLICY (F-02) — ONE decision covering BOTH the raw attribute and the
        # browser-resolved URL. A link whose displayed text matches the load but whose href is
        # foreign dies here, before any identity binding can admit it.
        #
        # It is one function on purpose. For an ordinary absolute href the attribute check and the
        # resolved check see the same URL, so a mutant that removed only ONE of them would
        # reintroduce no defect and be reported as a MISS while proving nothing — precisely the trap
        # `scheme_refusal_reason` already documents for the scheme rules. Keeping the whole decision
        # in one place lets the mutation battery remove it in a single edit and watch a guard fire.
        origin_refusal = link_origin_refusal(
            href, resolved, url_filter=url_filter, established_origin=established_origin)
        if origin_refusal:
            last_reason = origin_refusal
            continue
        if resolved:
    ...
    chosen = preferred[0]
    origin = parse_origin(established_origin) if established_origin else None
    # The reason names the checks that ACTUALLY RAN (F-02 item 7). It may never claim a domain
    # allowlist that was not configured, nor an origin check that did not happen.
    return chosen, (
        "provenance-bound load-detail link: identity-bound by "
        f"{chosen.bound_by}, observational route family, plain document anchor, and same-origin as "
        f"the established {origin.as_string() if origin else '<none>'}"
        + (f" (also matched the configured TMS domain filter {url_filter!r})" if url_filter else
           " (no additional TMS domain filter configured; the parsed-origin check is what admitted it)")
    )
...
def navigation_target_is_allowed(
    url: str,
    url_filter: str | None = None,
    *,
    established_origin: str | None = None,
) -> tuple[bool, str]:
    """Is `url` a document this surface may fetch? Returns (allowed, reason).

    ORIGIN SAFETY DOES NOT DEPEND ON THE OPTIONAL TEXTUAL FILTER (F-02). The authority here is
    `established_origin` — the parsed origin the OPERATOR established, never one a page published.
    `url_filter` is an ADDITIONAL narrowing, never the thing that makes navigation safe: with no
    established origin this function refuses, whatever the filter says.

    The decision, in order:

      1. an unparseable, userinfo-bearing, or non-`http(s)` target is refused outright;
      2. a genuinely RELATIVE target (`/path`, `?q`, `#frag`) stays on the established origin BY
         CONSTRUCTION — but it still requires that an origin has been established, because
         "relative to nothing" is not a safety property. Note `//host/path` is NOT relative: it is
         scheme-relative and changes host, so it takes the absolute path below;
      3. an ABSOLUTE target must match the established origin on all three normalized components —
         scheme, hostname, effective port — compared as parsed components, never as substrings;
      4. when a `url_filter` is ALSO configured, the target must satisfy it too (narrowing only).

    Pure and total, so the decision is unit-testable without a browser and the refusal reason is
    part of the contract rather than a log line. Every reason states the check that actually ran.
    """
    u = (url or "").strip()
    if not u:
    ...
    if established is None:
        # FAIL CLOSED. This is the F-02 correction: an absent or malformed origin configuration
        # must refuse navigation that could leave the TMS origin, never allow everything.
        return False, (
            "no established origin: this surface refuses navigation until the operator establishes "
            "one. An absent or malformed origin configuration fails CLOSED — it is not a licence to "
            "follow any target the page publishes."
        )

    if _is_relative_reference(u):
        return True, (
            f"relative target resolves against the established origin {established.as_string()} "
            f"(no host component to change)"
        )

    # A SCHEME-RELATIVE reference (`//host/path`) inherits the established scheme and then names a
    # host of its own. Resolve it that way so the decision — and the REASON — is the honest one:
    # this is an origin comparison, not a parse failure. `//secure.tms.test/x` is same-origin;
    # `//evil.example/x` is refused as cross-origin, which is what it is.
    absolute = f"{established.scheme}:{u}" if u.startswith("//") else u
    candidate = parse_origin(absolute)
    if candidate is None:
        # Name the SPECIFIC defect. "refused" with a three-way guess is not an auditable reason.
        return False, f"{u[:80]!r} {_origin_defect(absolute)} — refused"
    if candidate != established:
        return False, (
            f"cross-origin navigation refused: {candidate.as_string()} is not the established "
            f"origin {established.as_string()} (compared as parsed scheme/host/port, not text)"
        )
    if url_filter and not url_matches_filter(u, url_filter):
        return False, (
            f"{u[:80]!r} is on the established origin but not on the additionally configured TMS "
            f"domain filter ({url_filter!r})"
        )
    reason = f"same-origin as the established {established.as_string()}"
    return True, (reason + f", and on the configured TMS domain filter ({url_filter!r})"
                  if url_filter else reason)
...
class ReadOnlyCdpNavigator:
    ...
    __slots__ = ("__weakref__", "cdp_url", "url_filter", "timeout", "settle_seconds",
                 "observer", "__channel", "__context_seq", "last_refusal", "__origin")
    ...
    def __init__(
        ...
        *,
        ...
        # THE ESTABLISHED ORIGIN (F-02). Set from OPERATOR CONFIGURATION only — either handed in
        # here, or taken from the first `visit()`, which fetches an operator-configured entry URL.
        # A page-published link can never establish or change it, so no document this surface reads
        # can widen where it is allowed to go. Until it is set, navigation of any kind fails closed.
        self.__origin: Origin | None = parse_origin(allowed_origin) if allowed_origin else None
        if allowed_origin and self.__origin is None:
            raise ReadOnlyCdpError(
                f"refused: allowed_origin={allowed_origin!r} has no parseable http(s) origin. A "
                "malformed origin configuration fails CLOSED rather than degrading to 'allow all'."
            )
    ...
    def visit(self, url: str) -> bool:
        ...
        Scheme-, origin- and route-checked. This is the only entry by a target the page did not
        itself publish, because a run has to start somewhere; it is configuration, never model
        output — which is precisely why it, and only it, may ESTABLISH the origin when none was
        configured. Once established, the origin is immutable for this navigator's lifetime: a
        later `visit()` to a different origin is refused rather than silently re-pointing the
        surface.
        """
        if self.__origin is None:
            established = parse_origin(url)
            if established is None:
                raise ReadOnlyCdpError(
                    f"refused navigation (operator-configured entry URL): {str(url)[:80]!r} has no "
                    "parseable http(s) origin (malformed host, unsupported scheme, or embedded "
                    "credentials), so it cannot establish one either."
                )
            if self.url_filter and not url_matches_filter(url, self.url_filter):
                raise ReadOnlyCdpError(
                    f"refused navigation (operator-configured entry URL): {str(url)[:80]!r} is not "
                    f"on the configured TMS domain filter ({self.url_filter!r})"
                )
            self.__origin = established
    ...
    @property
    def established_origin(self) -> str:
        """The normalized origin this surface is confined to, or "" when none is established yet
        (in which case every navigation and every link selection fails closed)."""
        return self.__origin.as_string() if self.__origin is not None else ""
    ...
    def detail_link_for_load(self, load_id: str) -> ObservedLoadLink | None:
        ...
        link, reason = select_load_detail_link(
            payload, load_id, context_seq=self.__context_seq,
    ...
    def follow(self, link: ObservedLoadLink) -> bool:
        ...
        current, reason = select_load_detail_link(
            self.observer.load_row_links(link.load_id), link.load_id,
            context_seq=self.__context_seq,
            established_origin=self.established_origin, url_filter=self.url_filter,
        )
        if current is None:
            raise ReadOnlyCdpError(
        ...
        if landed:
            allowed, why = navigation_target_is_allowed(
                landed, self.url_filter, established_origin=self.established_origin)
            if not allowed:
    ...
    def _navigate(self, url: str, *, origin: str) -> bool:
        allowed, reason = navigation_target_is_allowed(
            url, self.url_filter, established_origin=self.established_origin)
        if not allowed:
        ...
        smuggled = embedded_foreign_url_reason(url, self.established_origin)
        if smuggled:
            raise ReadOnlyCdpError(f"refused navigation ({origin}): {smuggled}")
        if self.__channel is None:
            raise ReadOnlyCdpError("navigation session is not connected")
