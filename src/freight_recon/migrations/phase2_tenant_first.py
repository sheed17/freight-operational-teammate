...
def migrate(db: str, *, assertion: "OwnerAssertion | None" = None,
    ...
    try:
        ...
        # FKs into tenant_humans (M1, the decision-human and the AMBIGUOUS/CONFLICTING owner) and
        # self-FKs for the correction/supersession lineage. Created after M5 and M1. Idempotent.
        from .phase6_identity_binding_claims import (
            create_phase6_identity_binding_claims_schema)

        for step in create_phase6_identity_binding_claims_schema(conn, now=_now()):
        ...
        # M10's Compensation: `compensations` holds FKs into tenant_humans (M1), effect_grants (M3),
        # pipeline_instances (M2) and approvals (M4). Created after them all. A fresh database is built
        # with it directly; this brings a P2-shaped database to the same shape. Idempotent.
        from .phase6_compensations import create_phase6_compensations_schema

        for step in create_phase6_compensations_schema(conn, now=_now()):
            _mark(conn, f"phase6cm:{step}")
        conn.commit()

        # M11's Policy: `policies` holds FKs into tenant_humans (M1, author + activator), approvals (M4,
        # the governed-change approval) and a self-FK for supersession, and its Policy Owner singularity
        # index is created ON tenant_humans (M1). Created after them all. A fresh database is built with
        # it directly; this brings a P2-shaped database to the same shape. Idempotent.
        from .phase6_policies import create_phase6_policies_schema

        for step in create_phase6_policies_schema(conn, now=_now()):
        ...
        if rep.outcome in (CANONICAL_READY, MIGRATION_COMPLETE_RESTART_SAFE):
            ...
            from .phase6_compensations import stamp_phase6_compensations_version
