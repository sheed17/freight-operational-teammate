...
def apply_review_action(store: WorkflowStore, request: ReviewActionRequest) -> ReviewActionResult:
    ...
    if request.decision == ReviewDecision.APPROVE_EXPECTED_AMOUNT:
        ...
        return _result(
            ...
            updated,
            request,
            "Approved expected amount {amount} by {actor}",
            draft_follow_up_required=True,
        )
    ...
    if request.decision == ReviewDecision.APPROVE_FULL_AMOUNT:
        ...
        return _result(run, updated, request, "Approved full amount {amount} by {actor}")
...
def _payload(request: ReviewActionRequest) -> dict:
    return {
...
def _result(
    ...
    *,
    ...
    return ReviewActionResult(
        ...
        actor=request.actor,
        mutation_text=template.format(actor=request.actor, amount=amount).strip(),
