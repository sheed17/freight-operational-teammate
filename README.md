...
| **Transition/event completeness** — G2 is adjudicated and the contract is mechanised, but **7 transitions perform durable writes and name no event outright** (`PL-7a`, `AP-9`, `CF-7`, `EC-7`, `PO-2`, `PO-3`, `RU-8`). A row claiming it merely *consumes* another machine's event must now **prove** that relationship from structured columns (`CONSUMES-VALID`) — U5.1's first candidate was rejected because the claim went unchecked. The earlier "24-name-no-event" figure and the "121/13" split were never correct and are both retired | ### **PARTIALLY DISCHARGED** — naming those seven events is **founder/architect authority**, so **P5**'s event content stays blocked ([audit](docs/implementation/TRANSITION-EVENT-AUDIT.yaml)) |
