...
> ### **All 105 canonical emitted event names × all event versions. A `STRUCTURAL` case (`AC-EVT-000`) asserts a BIJECTION between the implementation's emitted-event registry and the canonical 105 — an undefined event or an unemitted contract fails the build.**
> ### **The oracle is EXACT SET EQUALITY of event names, not a count. A count match with different members MUST fail.**
> *(Amendment 2026-08-12, founder/architect authority: **98 → 105**. Seven events — `AutonomousAdmissionRecorded`(PL-7a), `ApprovalFrozen`(AP-9), `ConflictPartyAttached`(CF-7), `ExceptionSeverityChanged`(EC-7), `PolicySubmitted`(PO-2), `PolicyApproved`(PO-3), `RuleExpired`(RU-8) — were minted to discharge the seven recorded `GR-2` obligations. `AC-EVT-008` is the reason `ApprovalFrozen` EMITS rather than deriving: a rebuild that inferred `frozen` from an absence would reproduce a REUSABLE approval.)*
> *(Errata 2026-07-16: corrected from 92. The canonical event list (`events/registry.md` §3) enumerated **98** across F1–F13 at that time. F14's 13 audit/security events are counted separately and were correct; F15 is a lens and declares no contracts; `TimerFired` is a TRIGGER, not an emitted event, and is excluded. See `docs/implementation/canonical-corpus-errata-review.md`.)*

## Per-event mandatory assertions
| # | Assertion | Oracle |
|---|---|---|
| **AC-EVT-001** | every event uses the **canonical envelope** | schema assertion over all 105 |
| **AC-EVT-002** | ### **`tenant_id` mandatory AND first in partition identity** | reject any event without it; partition-key probe |
| **AC-EVT-003** | ### **every producer transition emits its required event** (134→105 map) | the outbox row exists in the transition's commit |
