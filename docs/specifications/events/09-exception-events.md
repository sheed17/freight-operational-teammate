...
| **`ExceptionSeverityChanged`** EC-7 | proves an open exception's severity was REASSESSED, carrying the value it moved from, the value it moved to, and who moved it · ¬proves resolution, ¬changes state | ### `severity`(R, new), `previous_severity`(R, old), `changed_by`(R), `reason`(R) | Oversight (the queue re-ranks); M9 (EC-4/EC-5 ageing and escalation thresholds) | ### **`ExceptionRaised` records severity at creation; EC-7 mutates it, so without this event a rebuild reproduces the ORIGINAL severity and can UNDER-STATE the live one** — and a Sev-0 exception auto-engages the brake (M9 §30), so that under-statement is a safety loss, not a cosmetic one. `previous_severity` makes a missing link DETECTABLE in a fold instead of silently absorbed. `actor_type ∈ {human, system}` — ### **never `model` (GR-7)**; `test_ev_exceptionseveritychanged_rebuilds_the_current_severity` |
