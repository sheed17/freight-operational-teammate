...
| **`RuleExpired`** RU-8 | proves a narrowing rule reached its TTL and ### **left the ACTIVE set** · ¬proves authority auto-broadened, ¬proves anyone confirmed the broadening | `rule_id`(R), `rule_version`(R), `expired_at`(R) | checkpoint (step 6 — which rules are ACTIVE); M9 (human-confirmation Exception) | ### **the exact counterpart of M11's `PolicyExpired` (PO-7), which F12 previously lacked.** Rule expiry changes which rules are ACTIVE, which changes checkpoint evaluation — a fact replay could not reconstruct before, because the raised Exception records a DIFFERENT aggregate and says nothing about THIS rule. ### **its expiry BROADENS ⇒ a human is still required at expiry**; `test_ev_ruleexpired_leaves_the_active_set_and_needs_a_human` |
