...
*(Amendment 2026-08-17 — **FOUNDER-DIRECTED, P6-D11**. §8's strict per-aggregate ordering is clarified to what it has always said — **ORDER, never CONTIGUITY** — and one **additive, optional** envelope field, `previous_aggregate_version`, is added to §1 so a strict-order consumer can tell an intentionally silent version from a lost event. ### **No event was minted, renamed, re-attributed or changed in meaning; §3 still enumerates 105 emitted contracts; no transition's classification moved.** The change is additive under §6, so `event_version` and `schema_version` are unchanged and every historical event stays readable and byte-identical.)*
...
| ### `previous_aggregate_version` | ### **C** | I | ### **the `aggregate_version` of the event this producer emitted IMMEDIATELY BEFORE this one on THIS aggregate's stream; `0` for the stream's first event.** Applicable — and therefore **REQUIRED** — for every producer emitting on a **strict-order** aggregate (§8). It is what lets a consumer tell *"the earlier event has not arrived yet"* from *"there is no earlier event to wait for"*, which are indistinguishable from an absence. (P6-D11.) |
...
- ### **STRICT MEANS *ORDER*. IT HAS NEVER MEANT *CONTIGUOUS*, AND IT CANNOT** *(P6-D11)*. `GR-2` is discharged by a **co-commit**: a transition whose canonical event belongs to ANOTHER machine's aggregate advances its own aggregate's version and emits **nothing** on its own stream. Eight of M2's twenty-five §14 rows are exactly that (`PL-6`, `PL-9`, `PL-10`, `PL-10f`, `PL-10u`, `PL-11`, `PL-11c`, `PL-15`), and `PL-11d` is a ninth non-emission on `pipeline_instance`. ### **A version with no event on a stream is therefore NORMAL, not a loss.** A consumer that treats every missing version as an unarrived event parks at the first one **and never unparks**, because nothing will ever fill it.
- ### **SO THE SUCCESSOR DECLARES WHAT IT FOLLOWS, AND THE CONSUMER STOPS INFERRING.** Every producer on a strict-order aggregate sets §1's `previous_aggregate_version`. **The consumer's rule is: block iff `previous_aggregate_version` is ABOVE its applied high-water mark** — never on the mere absence of a version number. An event that declares no predecessor (every historical event, every order-tolerant producer) falls back to contiguity: ### **absence may never be read as "there is nothing before me".**
- ### **THE BLOCK THAT SHOULD HAPPEN STILL HAPPENS.** A lost or reordered event is still caught, because ITS successor names a predecessor the consumer has not applied; the park, its arrival order and its M-26 TTL are unchanged. Only the **intentional** non-emission stops blocking, and only because it is now **positively declared to be nothing** — ER-16's principle one level down: *a fact is reconstructed from POSITIVE evidence, never from an absence.*
- ### **THE LINK IS DERIVED FROM THE EMITTED RECORD AND VERIFIED AT THE BOUNDARY THAT OWNS IT** — the transactional outbox, in the same transaction as the state write. It is **not** a second sequence beside `aggregate_version`: the ordering key stays `(tenant_id, aggregate_id, aggregate_version)`, and a producer declaring a predecessor its own emitted history does not hold is **REFUSED before the insert**, so a wrong link cannot tell a consumer to apply past a real event.
- ### **A STRICT-ORDER CONSUMER MUST CONSUME THE *COMPLETE* AGGREGATE STREAM, NEVER A FAMILY SUBSET** *(P6-D11, review F-3 / adjudication A-3)*. The predecessor chain is formed over **every event emitted on the aggregate**, not over the events of any one family, so a consumer that filters by family and then applies the ordering rule can filter out its own predecessor. ### **The live case is `IllegalTransitionAttempted`:** it is an **F14 order-tolerant** contract, but it rides on the **strict-order** `pipeline_instance` F2 aggregate at the attempt's unchanged version — so when it lands at a version a `CONSUMES` row left silent it becomes the **sole occupant of that version and therefore the declared predecessor** of the next F2 event. A consumer subscribing to F2 alone then blocks on a predecessor it deliberately discarded, and **never unblocks** — the same permanent silent stall `previous_aggregate_version` exists to prevent, reintroduced from the consumer side. ### **Family classification (§8's strict / order-tolerant split) governs what ORDERING GUARANTEE a family's own stream carries. It has never governed what a consumer may SKIP on someone else's aggregate**, and an order-tolerant event ordered inside a strict aggregate is canon rather than an accident — the universal ordering key `(tenant_id, aggregate_id, aggregate_version)` holds within one aggregate regardless of family. ### **This constraint is satisfiable, and it is what made the ORDER reading necessary:** under CONTIGUITY a strict consumer would have had to observe an event at *every* version, which canonical design makes impossible; under ORDER it need only observe **every emitted event on the aggregate**, which is the ordinary meaning of strict per-aggregate ordering and was proven end to end against a real attempt. **No new sequencing mechanism is introduced by this rule** — it is a requirement on the SUBSCRIPTION, not on the envelope.
