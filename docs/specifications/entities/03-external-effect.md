...
12. **Enums.** `state ∈ {GRANTED, CLAIMED, ATTEMPTED, VERIFIED, FAILED, EXPIRED_UNCLAIMED, UNKNOWN_OUTCOME}` (spec §12.3). Terminal: `VERIFIED, FAILED, EXPIRED_UNCLAIMED`. ### **Non-terminal, human-owned: `UNKNOWN_OUTCOME`.** `verification_outcome` ∈ the 8-member canonical registry. `unknown_reason` ∈ its 3 members.
