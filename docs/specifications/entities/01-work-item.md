...
17. **Uniqueness constraints.** PK `(tenant_id, work_item_id)`. No business-natural uniqueness (two distinct obligations about the same load are two Work Items). ### **SD-10 — deterministic intent on cross-trigger duplication:** ### **Work Items are NOT deduplicated across trigger sources.** Two triggers implying the *same* obligation (an inbound email AND an AR sweep both flagging load 4471 for billing) legitimately create **two Work Items** — they are two independent *"we should act"* signals with possibly different owners. ### **Double-ACTION is prevented downstream, not here: both Work Items spawn Pipeline Instances that share ONE `commit_key`, and the Layer-1 reservation absorbs the second proposal into the first (M-29) — so at most one card, one approval, one effect.** When the shared effect closes, ### **each Work Item observes `PipelineClosed` for its `commit_key` and closes with the same `decision_ref`** (a redundant Work Item is closed by the same closure event, not left dangling). *(Why already implied: spec §13 makes the Work Item the unit of intent and the Pipeline the unit of the effect, with commit-key exclusion at the effect layer — §16.1/M-29. This states that intent-level duplication is expected and effect-level exclusion resolves it. **No new concept, no dedup mechanism invented.**)*
