...
| **Safe interim behaviour** | Fail-closed. Unknown classification is a build failure, never a pass or a skip; the seven are recorded as open obligations that cannot be silently discharged; and until `AP-9` is decided, any approval with an unresolved `OutcomeUnknown` is treated as frozen. |
