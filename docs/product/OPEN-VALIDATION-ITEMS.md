...
| | |
|---|---|
| **Question** | ### **SETTLED, AND THE NAMING QUESTION IS ANSWERED.** G2 ruled that a producer transition is one declared in `events/registry.md` §3 and that completeness is `GR-2` over durable writes, and all 134 rows are mechanically classified ([`TRANSITION-EVENT-AUDIT.yaml`](../implementation/TRANSITION-EVENT-AUDIT.yaml)). The seven transitions that performed durable writes no canonical event recorded — `PL-7a`, `AP-9`, `CF-7`, `EC-7`, `PO-2`, `PO-3`, `RU-8` — were given seven **minted** canonical events under founder/architect authority on 2026-08-12, taking the registry 98 → 105 (`AC-TRACE-000` now asserts 105/105). ### **WHAT IS LEFT HERE IS NOT A NAMING QUESTION.** The recorded G2 residuals `G2-D4`, `G2-D6`, `G2-D8`, `G2-D9` and `G2-D10` stay OPEN, and none of them is discharged by the mint. |
| **Affected workflow** | All |
| **Affected entity/state** | State machines ↔ event contracts |
| **Implementation impact** | ### **No longer blocks P5's event content.** Each of the seven WAS a live `GR-2` violation: the durable write happened, no event recorded it, and replay reconstructed a state history with holes. `AP-9` was the sharpest — it writes `frozen=true`, a guard input to an ILLEGAL determination, so a rebuild reconstructed an approval that was **not** frozen, i.e. **reusable**, *less safe than the original*. That is why `ApprovalFrozen` **EMITS** rather than deriving `frozen` from the absence of a `RealityEstablished` (registry `ER-16`). P5's event content is now unblocked by the specification and blocked only by not having been built. |
