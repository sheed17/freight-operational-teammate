...
| **Still owed** | **M9–M13**, 37 of the 134 transitions, gate **G1**, `AC-SAFE-028`. **M9 — the Exception — is the next build checkpoint.** (The transition figure is derived, not carried: M1's 14 + M2's 25 + M3's 13 + M4's 11 + M5's 8 + M6's 11 + M7's 7 + M8's 8 = 97 of 134 written, so 37 remain — re-derived mechanically at this landing by parsing §14 of all thirteen machine files and counting rows, which discovered 13 files and 134 rows, not by carrying a prior figure. The `45` this cell read until the `P6-CP-8` landing was the post-M7 figure, the `52` before that the post-M6 figure, the `63` before that the post-M5 figure, the `71` before that the post-M4 figure, and the `95` before that the post-M2 figure.) |
...
| **Posture** | M1, M2, M3, M4, M5, M6, M7 and M8 all **ship dark**: zero production importers; M2's/M3's import closure reaches no effect-capable adapter, nothing joins M4 to an outbound channel, and the only thing outside the package that reaches M5, M6, M7 or M8 is its own probe. **No production effect or integration is enabled by any of them**, and `checkpoint.py` remains the sole minter of a gate decision. |
...
Landing M8 scores no P6 criterion. **The next build checkpoint is M9 — the Exception.**
