...
| — | **Transition/event completeness — G2 ADJUDICATED, PARTIALLY DISCHARGED.** The predicate is settled and mechanised (interpretation C, HYBRID): a *producer transition* is one declared in `events/registry.md` §3 — **110 of the 134 rows; the other 24 are non-producer transitions** — and completeness is `GR-2` over durable writes. All 134 rows now carry structured classification (110 PRODUCER · 9 CONSUMES · 6 NON_PRODUCING · 2 DELEGATES_TO · 7 EVENT_REQUIRED); prose is never a classifier and an undecidable row fails the build. `EF-3` is re-attributed to the **existing** `EffectExecuted`. ### **`CONSUMES` IS PROVEN, NOT ASSERTED.** A durable-writing consumer must satisfy `CONSUMES-VALID`: a **bidirectionally declared co-commit** in both rows' `Writes` cells, a **different machine**, **not mutually exclusive** with the owner, and every persisted field carried by a consumed event's `state-machines/registry.md` §5 payload — undecidable **fails the build**. The first U5.1 candidate's version of this class was self-certifying and was **rejected**: it let `AP-9` be relabelled `CONSUMES:BrakeReleased` and stay green. `PL-15x` and `IB-5x` moved to `NON_PRODUCING` (their event's producer is the *rule* `GR-1`, not a transition). ### **WHAT REMAINS: 7 transitions perform durable writes and name no event outright** — `PL-7a`, `AP-9`, `CF-7`, `EC-7`, `PO-2`, `PO-3`, `RU-8` — each a live `GR-2` violation, each recorded with its semantic obligation and **no invented name**, in [`TRANSITION-EVENT-AUDIT.yaml`](TRANSITION-EVENT-AUDIT.yaml). Naming them changes the frozen 98-event registry and is founder/architect authority. The retired "24-name-no-event" figure and the retired "121/13" split were never correct | ### **PARTIALLY DISCHARGED — 7 obligations OPEN** | **G2 — founder/architect event naming, before P5 event content** |
...
> ### **P5 CARRIES ITS OWN BLOCKER. G2 IS NOW ADJUDICATED AND PARTIALLY DISCHARGED — AND STILL
> BINDING.** The G2 contract is settled and mechanised, `EF-3` is re-attributed to the existing
> `EffectExecuted`, and all 134 transitions are structurally classified. But **seven durable writes
> still have no canonical event** (`PL-7a`, `AP-9`, `CF-7`, `EC-7`, `PO-2`, `PO-3`, `RU-8`), and
> minting those names changes the frozen 98-event registry — **founder/architect authority, which no
> builder, reviewer or adjudicator holds**. ### **P5's event content may not be written until those
> seven are decided.** P4's completion did not touch any of this; the P4 final adjudication says so
> explicitly. It is recorded in P5's `validation_blockers` and in
