...
> ### **THAT FINALIZER HAS SINCE RUN — `dd9808e`.** ### **SUPERSEDED WORDING, kept so it is
> recognisable if it returns (§5 rule 20):** this read *"WHAT REMAINS IS EXACTLY ONE CANONICAL
> FINALIZER RUN — for THIS landing, and it is the ONLY finalizer owed anywhere in P6."* It was true
> when written and is **false now**. `dd9808e` records a completed canonical finalizer run for the
> `P6-D11` landing: `SUITE-RESULT.json` and `GATE-RESULT.json` at that commit both bind
> `fa6481eb2e47f6f4` / tree `bd3aa1b500c9ff6c` at **3051 / 0 / 1** with the clean-clone gate
> `passed: true`, the receipt's `payload_sha256` recomputes exactly (so it was not hand-edited), and
> the commit touches only `STATUS_METADATA_FILES`. A further pair (`15a0fe5` → `d1b47c2`) has since
> been finalized on top. ### **NO P6 FINALIZER IS OWED.** **`M3` is no longer blocked on a
> finalizer — it is blocked on the `R-21b` procedure repair below.**
...
content_commit: 15a0fe5c7315ef244868b2678b8778bedb50e5ec
content_tree: 134a9b6b9f868d2da069036802b1a077bfcb418c
suite_passed: 3051
suite_failed: 0
suite_skipped: 1
```
...
|---|---|---|---|
| **R-07** | Ungated live-write paths | ### **CONTAINED** — recorded in [`phase-0-baseline-manifest.yaml`](phase-0-baseline-manifest.yaml) (`expected_legacy_paths.status: CONTAINED`) with the mechanism named | ### **CLOSED** by the separate content commit |
...
| — | ### **`R-21b` — THE INTEGRATION PROCEDURE WAS UNEXECUTABLE, AND IT IS REPAIRED.** `integration-topology-procedure.md` §3 prescribed *"rebase onto `main`'s tip, collapse to ONE content commit, then finalize"*. With `origin/main`'s tip a **merge commit** (`152574e`, whose first-parent spine is a throwaway history — all real work arrived as **second** parents) that intermediate state fails **both** live guards, and `finalize_status.py` validates before it writes, so Step 4 could never run. The root cause was the topology guard expressing the invariant as a **parent count** — rejecting a merge at `HEAD^`, which in `FINALIZED` is the **content** commit, a case §2 never constrained. Because integrating a diverged `origin/main` necessarily puts a merge on the first-parent chain, the recorded anchor was **trapped below it**: every position where `repo_state()` bound was rejected on parent count, every position accepted was stale. ### **THE GUARD NOW ASSERTS THE TWO PROPERTIES THAT MATTER** — the first-parent chain binds to the certified pair, and the status-metadata commit has **exactly one parent** — and the hermetic scenarios evaluate them **at the finalizer-time state** against the real `repo_state()`, which the previous rehearsal never did (it hand-wrote its metadata commit and looked only at the final pair). ### **THE RELAXATION IS CONFINED TO ONE THING: the CONTENT commit is no longer required to have exactly ONE parent** — its parent count is still read, and still bounded, at **TWO**. A merge used as the **metadata** commit, an octopus metadata commit, a chain that fails to bind the recorded pair (the usual merge-button shape), stacked unfinalized content commits, and an **octopus CONTENT commit** are **all still rejected**, each by a test that fails when its rule is removed. What may now be accepted is narrow and conditional: a **two-parent** merge used as the **content** commit, and only where the certified chain binds, the state is `PRODUCING` and a finalizer is owed on it. ### **The octopus content commit was closed on review.** The first independent review of candidate `9a2ab11` constructed it (finding F-1): parents `(metadata, origin/main, arbitrary)`, chain binds, state `PRODUCING`, no topology error, and `git show` renders an **empty patch** — an unreviewed lineage riding in while every mechanical precondition passes. The remediation bounds the content commit at two parents and is mutation-proved three ways. ### **The cost is stated rather than hidden:** a button-created merge that preserves first-parentage is now indistinguishable from a legitimate integration, so the control for that class is the allow-only-rebase-merging repository setting rather than the topology guard. ### **NO HISTORY WAS REWRITTEN AND `main` WAS NOT TOUCHED** — integration stays fast-forward-only | ### **REPAIRED AND INDEPENDENTLY REVIEWED — `ACCEPT FOR SEPARATE TARGETED ADJUDICATION`, 8 nonblocking findings** ([report](r21b-independent-review-report-9a2ab11.md)). The review's two recommended landing conditions (**F-2** the procedure's own residual *"without relaxing anything"* overstatement, **F-3** no instruction to review a merge content commit by its first-parent diff) are APPLIED, and its **F-1** octopus gap is CLOSED rather than recorded. ### **STILL OWED: one separate targeted adjudication by a session outside every lineage, then exactly ONE canonical finalizer run** from a host that permits `socket.bind`. **Integration itself is still OPEN and is a founder-authorized push** | `test_integration_topology.py`, `R-21b` |
...
> [adjudication](p6-d11-targeted-adjudication-report-021a9a2.md)). ### **SUPERSEDED WORDING, kept so it is recognisable if it returns (§5 rule 20): it said the `P6-D11` landing still owed a finalizer. THAT FINALIZER RAN AT `dd9808e`** — receipts bind `fa6481eb2e47f6f4` / tree `bd3aa1b500c9ff6c` at 3051/0/1, gate `passed: true`, `payload_sha256` recomputes, and the commit touches only `STATUS_METADATA_FILES`. A further pair (`15a0fe5` → `d1b47c2`) has since been finalized on top. **No P6 finalizer is owed.**
> ### **`M3` is no longer gated on a finalizer — it is gated on the `R-21b` procedure repair,
> which is built and awaiting fresh independent review.**
>
> <details>
...
> the finalizer is closed. ### **SUPERSEDED WORDING, kept so it is recognisable if it returns (§5 rule 20): it said the `P6-D11` landing still owed a finalizer. THAT FINALIZER RAN AT `dd9808e`** — receipts bind `fa6481eb2e47f6f4` / tree `bd3aa1b500c9ff6c` at 3051/0/1, gate `passed: true`, `payload_sha256` recomputes, and the commit touches only `STATUS_METADATA_FILES`. A further pair (`15a0fe5` → `d1b47c2`) has since been finalized on top. **No P6 finalizer is owed.**
> The conditions that run named — a session in none of the build, review, adjudication or
> landing lineages, an exclusively-held `finalizer_lock`, one status-metadata commit touching
> only `STATUS_METADATA_FILES`, and a host that permits `socket.bind` and reaches pypi.org over
> untampered TLS — **remain binding on the NEXT finalizer**, which is the one the `R-21b`
> procedure repair will owe once it is independently reviewed and adjudicated.
> ### **`M3` MAY NOT BEGIN BEFORE THAT.** It will follow the
...
> ### **The branch is 48 ahead / 5 behind `origin/main` and `origin/main` is NOT an ancestor**, so
> integration requires merging it IN as one content commit (never a rebase — see `R-21b`), then one
> finalizer, then the fast-forward push. **No rewrite of `main` is required or authorized.**
...
| ### **`M3` — and any other consumer of M2's F2 stream** | ### **STILL BLOCKED — BUT NO LONGER ON A FINALIZER. THE `P6-D11` LANDING FINALIZER RAN AT `dd9808e`; `M3` IS NOW BLOCKED ON THE `R-21b` PROCEDURE REPAIR.** (1) `P6-CP-2` is **FULLY FINALIZED at `c2def38`** and owes **no** finalizer — the superseded claim that it owed one is corrected throughout this document (§5 rule 20). (2) ### **`P6-D11` IS RESOLVED, independently REVIEWED (ACCEPT), separately ADJUDICATED (ACCEPT) and LANDED** ([record](p6-d11-f2-ordering-contract-record.md) · [review](p6-d11-independent-review-report-021a9a2.md) · [adjudication](p6-d11-targeted-adjudication-report-021a9a2.md)); its finalizer **RAN at `dd9808e`** (receipts bind `fa6481eb2e47f6f4` / tree `bd3aa1b500c9ff6c`, 3051/0/1, gate `passed: true`, payload hash recomputes). ### **What now stands between here and `M3` is `R-21b`** — built, awaiting fresh independent review, then a separate adjudication, then **one** finalizer, by a session in none of the build, review, adjudication or landing lineages. (3) When `M3` begins it inherits **`P6-D24`** — its strict consumer must supply `drain_handler_for`, or a parked event leaves the park only by M-26 expiry — and **§8's complete-stream rule**: a strict-order consumer must consume the WHOLE aggregate stream, never a family subset, because an `IllegalTransitionAttempted` (F14) riding on the strict F2 aggregate can be the declared predecessor of the next F2 event. ### **The engineering boundary itself is closed:** §8 now states strict = ORDER, never contiguity, every strict-order producer declares `previous_aggregate_version`, and a real M3-shaped consumer reads a real M2 attempt end to end with nothing parked — while a genuinely lost event still parks. ### **SUPERSEDED WORDING (§5 rule 20), kept so it is recognisable if it returns:** *"`P6-D11` is a hard boundary:* eight of M2's 25 rows are `CONSUMES` — they advance the attempt's version and emit nothing on `pipeline_instance`, a **STRICT-ORDER** F2 family whose inbox parks any version above `applied+1` — so the stream has **measured** gaps that would **permanently park** the first real consumer, and that consumer is M3. The adjudicator named this explicitly: *"It must be resolved before M3, and it is **not** discharged by M2 landing."* Resolving it is a canonical decision about the F2 contract, **not** something a builder may improvise |
