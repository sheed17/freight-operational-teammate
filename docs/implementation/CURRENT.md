...
recorded the P6 PHASE-ACCEPTANCE AUTHORITY CORRECTION, which instantiated the seventeen-criterion
contract and scored none of it, and said "every result is `PENDING`". That was TRUE WHEN WRITTEN and
is FALSE NOW; it is REPLACED rather than deleted — [`CLAUDE.md`](../../CLAUDE.md) §5 rule 20.)*
...
deleted.)* All seventeen criteria are PASS, P6 is `COMPLETE`, and **P7 is the sole `READY` unit** —
which is a SELECTION and nothing more: **no P7 code exists, P7 is `NOT_STARTED` / `NO_CHECKPOINT`,
and not one P7 criterion is scored.**
...
| **Scored — 17/17** | `criteria_scored` is still `[]` on all **thirteen** landed checkpoints, and that is unchanged: **a checkpoint is a landed increment, never a phase acceptance.** What accepted P6 is the separate phase `acceptance_criteria` block, scored **17/17 PASS** by a reviewer outside the build lineage on CI run `34314374504` — so the registry now reads `status: COMPLETE` / `execution_state: COMPLETE` / `checkpoint_state: PHASE_ACCEPTANCE_COMPLETE`, and **P7 is the sole `READY` unit, `NOT_STARTED` / `NO_CHECKPOINT`**. *(Until this commit this row read "**Not scored** — … P6 has not reached phase acceptance … **P7 stays `BLOCKED` / `NOT_STARTED`**", and warned that reading "all 13 landed" as "P6 COMPLETE" was the mistake it existed to prevent. That warning was correct and still is: **P6 is complete because it was independently scored, NOT because thirteen machines landed.** REPLACED rather than deleted — [`CLAUDE.md`](../../CLAUDE.md) §5 rule 20.)* |
