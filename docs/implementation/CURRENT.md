...
7. **EP-14 cut (this session).** `scripts/read_tms_browser_use.py` imports only
...
8. **EP-1 READ half cut (this session, U4.11). The write half is NOT cut.** The callback server
   ...
   **Edge counts are unchanged (violations 1, detection 14)** — deliberately. The `cdp_actuator`
   import survives because `_build_live_operation_router._build_agent` still constructs it for the
   autonomous browser WRITE. A guard now asserts that factory is the **only** construction site, so
   the residual is named and cannot quietly spread. Mutation battery **45/45** (added B36–B39,
   including "a second live write driver appears beside the residual").

So the EP-1 **write** conversion and flipping the gate to assert EMPTY remain — **R-07 stays OPEN**
under P4's own acceptance contract, on **one** residual violation edge.
