...
| **Still owed** | **M8–M13**, 45 of the 134 transitions, gate **G1**, `AC-SAFE-028`. **M8 — the Expectation — is the next build checkpoint.** (The transition figure is derived, not carried: M1's 14 + M2's 25 + M3's 13 + M4's 11 + M5's 8 + M6's 11 + M7's 7 = 89 of 134 written, so 45 remain — re-derived mechanically at this landing by parsing §14 of all thirteen machine files and counting rows, which discovered 13 files and 134 rows, not by carrying a prior figure. The `52` this cell read until the `P6-CP-7` landing was the post-M6 figure, the `63` before that the post-M5 figure, the `71` before that the post-M4 figure, and the `95` before that the post-M2 figure.) |
...
Landing M7 scores no P6 criterion. **The next build checkpoint is M8 — the Expectation.**
