...
| **Mutation** | `scripts/mutate_phase5_contracts.py` — ### **36/36 mutants caught**. Includes **two that edit the canonical specification itself** (a renamed contract, a consequential member removed) to prove the anti-drift guard is real, and **seven that mutate the PARSER** — the coverage hole the review named, since four real defects lived there and no original case could reach them. A parser case regenerates the data after mutating, because a parser defect is inert until the data is re-derived: the first run reported those five as MISSes and was right to |
