...
| **U0.10** | live-effect entry-point guard | `AC-SEC-013` | *(HISTORICAL — this frozen Phase-0 sequence records the state at P0; R-07 was recorded CONTAINED at P4)* ### **R-07 stays OPEN at P0** | n/a | n/a | ### **every effect-capable entry point is classified; at P0 NONE was contained** |
...
**P5:** U5.1 ### **the G2 specification/control correction** + the P5 acceptance-contract instantiation; U5.2 ### **the G2 founder/architect event mint** *(seven canonical events, 98 → 105)*; U5.3 ### **the 105 event contracts** + upcaster *(errata: was 92; founder/architect mint 2026-08-12: 98 → 105)* — depends on U5.7/U5.8; U5.4 ### **the `GC-1` golden corpus + pinned digest** ⇒ `AC-EVT-007/008`; U5.5 ### **sandboxed replay — effects structurally unreachable in replay** ⇒ `AC-EVT-011`; ### **U5.6 audit reconstruction (18-field explainability, beliefs-of-that-day)** ⇒ `AC-AUD-*`; U5.7 transactional outbox ⇒ `AC-RACE-006/007`; U5.8 dedup inbox ⇒ `AC-ADPT-010`; U5.9 ### **durable timers + the production PostgreSQL store, migrations and runtime** *(ADR-016 · M-36)* — depends on U5.7/U5.8.
> ### **U5.1 AND U5.2 ARE NOT THE OUTBOX AND THE INBOX — and in this file, until 2026-08-12, they were.** `IMPLEMENTATION-REGISTRY.yaml` records U5.1 and U5.2 as the two G2 units that were actually built, independently reviewed, separately adjudicated and finalized, and their preserved governance refs (`refs/preserve/p5-u51-*`, `refs/preserve/p5-u52*`) name those ids permanently. ### **Certified history is immutable, so nothing was renumbered.** The two still-unbuilt deliverables that had been carrying those ids were REASSIGNED to **U5.7** (transactional outbox) and **U5.8** (dedup inbox); both returned zero hits repository-wide — working tree and full commit history — before the reassignment. ### **U5.3 therefore depends on U5.7/U5.8: a lower-numbered unit depending on higher-numbered ones. That is deliberate. Dependency correctness outranks tidy digits, and renumbering further planned work to make the digits ascend would be a larger edit for a cosmetic gain.** Registry ↔ plan agreement is asserted mechanically, per id, by `eval/tests/test_phase0_identifiers.py::test_the_p5_unit_ids_name_the_same_work_in_the_registry_and_in_the_plan`.
> ### **`U5.9` WAS ADDED TO THE LINE ABOVE ON 2026-08-14 BY A RECORD CORRECTION, NOT BY NEW WORK.** The durable timers and the production PostgreSQL store, migrations and runtime **had already landed** (content commit `5a5b5e5`) carrying **no unit id at all** — the omission the independent P5 review recorded as part of **IR-R3**. `U5.9` was unused repository-wide, working tree and full history across every ref, before it was assigned. ### **Nothing was renumbered, nothing was built, and no acceptance criterion was scored by that correction** — all fourteen P5 criteria remain `PENDING` and are set only by a separate final adjudication.
