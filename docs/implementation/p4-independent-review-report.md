...
> **Preserved as received. This is evidence of a past moment, not status.** It reviewed candidate
> `95cf5af7d9eae19cba5ab2f0a745ef3c04858962` (tree `4b3dda20194a1f7de790a12912316a1cef25e819`) and
> returned **REJECT — REMEDIATION REQUIRED**. It does **not** review, describe or approve the
> remediated candidate that carries it, it adjudicates nothing, it marks P4 neither complete nor
> incomplete, and it does not close R-07. Current status: [`CURRENT.md`](CURRENT.md) · operating
> guide: [`../../CLAUDE.md`](../../CLAUDE.md).
>
> **BYTES.** Everything below this banner is the reviewer's report, unaltered. The banner is the
> only addition, and it is required by this repository's own control system
> (`test_false_green_defenses.py::test_historical_documents_disarm_before_any_stale_claim`, which
> refuses any tracked historical review that a grep-first reader could mistake for authority).
> The **byte-exact original**, with no banner, is preserved unmodified at
> `refs/preserve/p4-independent-review-95cf5af` — a commit whose parent is the rejected candidate
> itself — and its SHA-256 is:
>
> ```
