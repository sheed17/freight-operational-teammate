...
| 15 | **Outbox/Inbox** *(ids reassigned — see the note below the table)* | ### **none** | ### **ABSENT** | transactional outbox + dedup inbox | `AC-RACE-006..009`, `AC-EVT-003` | ### **U5.7/U5.8** | U2.1 | **G2** *(+G4 safety)* |
| 16 | **Event contracts** | ad-hoc audit rows | **PARTIAL** | ### **105 events** + envelope + versions | `AC-EVT-*` | U5.3 | ### **U5.7/U5.8** | **G2** |
...
> ### **NOTE ON ROWS 15 AND 16 — THE OUTBOX AND THE DEDUP INBOX ARE `U5.7` AND `U5.8`, NOT `U5.1` AND `U5.2`** *(reassigned 2026-08-12)*. This matrix, and `pr-sequence.md`, used `U5.1`/`U5.2` for the two of them while `IMPLEMENTATION-REGISTRY.yaml` recorded those ids as two G2 units that had actually been built, independently reviewed, separately adjudicated and finalized. ### **Certified history is immutable, so nothing was renumbered:** the unbuilt deliverables moved to `U5.7`/`U5.8`, both unused repository-wide beforehand. Row 16's `Depends on` moved with them, which is why ### **`U5.3` depends on `U5.7`/`U5.8` — a lower-numbered unit depending on higher-numbered ones, deliberately, because dependency correctness outranks tidy digits.** Both halves are held against the registry by `eval/tests/test_phase0_identifiers.py::test_the_p5_unit_ids_name_the_same_work_in_the_registry_and_in_the_plan`.
