...
## S4g — Phase-5 durable timers *(canonical, not legacy)*

**Modules:** `event_timers.py` · `migrations/phase5_durable_timers.py`

> ### **HOW NEYMA NOTICES THAT SOMETHING DID NOT HAPPEN.** Almost everything that goes wrong in
> freight is a SILENCE: the POD never arrives, the carrier never checks in, the appointment window
> passes with no update, detention accrues while nobody notices. A durable timer is how a MISSING
> event becomes an observable one — `ExpectationOverdue` (EX-3), `ExceptionAgeing`/`ExceptionEscalated`
> (EC-4/EC-5), `WorkEscalated` (WI-10), `ApprovalExpired` (AP-3) and `GrantExpired` (EF-2x,
> *"nothing happened"*) are all to be driven by a `TimerFired` trigger, and none can exist without
> one. ### **STATED AS A DESTINATION, NOT AN ACHIEVEMENT.** Those six are P6 machine transitions
> and NONE of them is wired to this service yet. `GrantExpired` in particular is produced TODAY by
> `checkpoint.py:expire_unclaimed`, which is a genuine sweep over `effect_grants.expires_at` — see
> **TIMER-D1** below. Writing "are driven by" would have described the finished system as though it
> already existed.

| | |
|---|---|
| **Current responsibility** | The `durable_timers` schedule and its firing relay: scheduling that commits with the obligation, cancellation that commits with the terminal transition, at-least-once firing under lease exclusivity, and restart-safe deadlines |
| **M-36, structurally** | *"A timeout MUST be a durable timer emitting `TimerFired` — never a background sweep, never a scan for things that look old."* Asserted by `test_no_component_scans_for_staleness`, which the specification names outright: every module's SQL literals are parsed, and a query naming a business table AND inferring age from a RECORD-KEEPING timestamp is a finding. ### **A DEADLINE COLUMN IS NOT A SWEEP INDICATOR** — `pending_references.expires_at` is M-26's prescribed mechanism, written explicitly at park time; an earlier version of the guard flagged it, which would have condemned the mechanism the specification mandates |
| **The guard's one exemption, declared** | ### **`checkpoint.py:expire_unclaimed` IS A REAL SWEEP AND IS EXEMPTED BY NAME.** It scans `effect_grants` for lapsed `expires_at` and emits `GrantExpired`. It predates this work, it belongs to the P3 effect kernel, and converting it is a change to certified effect-authority code that this increment deliberately did not touch. The guard carries it in an EXACT-SET `KNOWN_SWEEP_EXEMPTIONS`, so a second sweep — or this one moving to another file — is a failure rather than a silent inheritance |
| **Current execution authority** | ### **NONE, AND THE BOUNDARY IS A SAFETY PROPERTY.** `TimerFired` is a TRIGGER, not one of the 105 canonical contracts (the corpus errata excludes it explicitly). This service says only *"the time you asked for has arrived"*; the MACHINE transitions by its own guard, and those machines are P6. GR-6 forbids any timer transition from moving an `UNKNOWN_OUTCOME`, and `NEEDS_VERIFICATION`/`COMPENSATION_FAILED`/an ACTIVE policy each name ANY `TimerFired` as ILLEGAL — a timer service that decided outcomes could violate all of them. It imports no `EventEnvelope`, no contract registry and no outbox, asserted structurally |
| **Durability** | The deadline is immutable once written (trigger, not convention — a deadline that can be edited can be quietly pushed out); FIRED and CANCELLED are terminal and can never return to SCHEDULED; rows are append-only, because a timer records that a deadline EXISTED. Scheduling refuses to run outside the caller's transaction, and so does cancelling — a cancellation committing separately leaves an obligation closed with a live deadline, or a cancelled deadline with the obligation still open |
| **Current risk** | Low, and bounded by having no production caller: nothing schedules a timer yet, because the transitions that would are P6's. SQLite only |
| **Canonical destination** | This IS the canonical destination (M-36, ADR-016 §2 "durable timers and scheduler") |
| **Disposition** | ### **KEEP** — canonical, forward-only. `schedule` and `cancel` may never gain a mode that runs outside a transaction; the immutability and terminal-state triggers may never be relaxed; and the service may never learn what a fired timer MEANS |
| **Target phase** | Implemented at **P5**; consumed by **P6**'s machines |
| **Compatibility requirement** | `TimerFired` stays a trigger and never becomes a canonical event; CANCELLED stays distinct from FIRED |
| **Deletion condition** | Not deletable — no permanent second timeout mechanism may exist beside it, and a sweep is exactly what M-36 forbids |
| **Evidence required before deletion** | n/a |
...
|---|---|---|
| **TIMER-D1** | `checkpoint.py:expire_unclaimed` remains a sweep, so `GrantExpired` is not yet timer-driven | The rewrite touches P3 certified effect-authority code to no product benefit until the P6 machine that consumes `TimerFired` exists. Converting it blind, with no consumer, would be a change to money-adjacent code justified by tidiness. It is exempted BY NAME rather than by pattern, so it cannot spread |

## S4h — Phase-5 production persistence *(canonical, not legacy)*

**Modules:** `persistence.py` · `migrations/postgres_p5.py` · **Gate:** `scripts/postgres_p5_gate.py`

> ### **P5's capability is "durable event execution (outbox/inbox/replay) ON production-grade
> persistence (PostgreSQL, migrations, durable timers)".** A green SCHEMA gate is not that: it
> proves five tables and their triggers exist in another database. What the contract asks is that
> the RUNTIME operates there — so `TransactionalOutbox`, `OutboxRelay`, `DedupInbox`,
> `DurableTimers` and `TimerRelay` are driven against a real PostgreSQL, through a real connection.

| | |
|---|---|
| **Current responsibility** | `persistence.py` presents the sqlite3 `Connection` API the certified runtime already speaks, and translates underneath. `migrations/postgres_p5.py` renders the P5 surface — 5 tables, 5 indexes, 8 invariants as `plpgsql` — for PostgreSQL |
| **Why a wrapper, not a rewrite** | The transport encodes hard-won semantics (emit refuses outside a transaction; the relay leases whole aggregates; consumption and the inbox row share one commit). Rewriting for a second dialect would put every one of those back in play. The runtime changed by a handful of lines: dialect-agnostic readiness checks, one portable upsert, and catching either integrity-error type |
| **### THE DEFECT THIS UNIT EXISTS TO HAVE CAUGHT** | `BEGIN IMMEDIATE` has **no PostgreSQL equivalent**. SQLite takes the write lock at BEGIN, which is what serialises the outbox's `MAX(sequence)+1` allocation — the runtime's own comment says so. Under PostgreSQL MVCC two emitters both read the same MAX and both commit. ### **MUTATION-PROVEN: with the advisory lock removed, eight concurrent emitters allocated sequence 3 SIX times and sequence 4 twice, raising ZERO errors.** Silent corruption of event ordering on the production store, invisible to every SQLite test. Handled by a per-tenant `pg_advisory_xact_lock`, released on commit or rollback |
| **Other real dialect differences** | `MAX(a,b)` is a scalar in SQLite and an aggregate in PostgreSQL — translated to `GREATEST`, but ONLY the two-argument form, because blanket replacement would turn a legitimate aggregate into a scalar over one value. PostgreSQL requires table-qualification on the right of `ON CONFLICT DO UPDATE SET` — fixed by making the SQL portable rather than adding a rewrite rule, since a rewrite rule for upsert bodies is a parser and a parser gets one case wrong later. `sqlite3.Row` supports BOTH name and positional access and the outbox uses both — emulated faithfully rather than changing certified code to suit the shim |
| **Current execution authority** | ### **None.** Persistence moves no money and grants no authority; the effect boundary is untouched. No pooling, no failover, no backup/PITR, no credential handling — ADR-016 assigns those to P11/P12, and the caller supplies the connection |
| **Evidence** | `scripts/postgres_p5_gate.py` — an explicitly invoked gate that EXECUTES against a real server and writes `POSTGRES-P5-GATE.json`. ### **It is a gate rather than pytest nodes deliberately:** the canonical suite must reproduce in a fresh clone with declared dependencies and nothing else, and a node that SKIPPED when no server answered would be the silent pass `APPROVED-SKIPS.yaml` exists to forbid — *"a skip is a test that did not run, and a test that did not run proves nothing"* |
| **Current risk** | Low. The SQLite path is byte-unchanged in behaviour (its batteries pass untouched), and the PostgreSQL path has no production caller yet |
| **Canonical destination** | This IS the canonical destination for P5 persistence (ADR-016 §1/§2) |
| **Disposition** | ### **KEEP** — canonical, forward-only. The advisory lock may never be removed without replacing the serialisation it provides; the two-argument `MAX` translation may never be broadened to the aggregate form; and `_Row` must keep BOTH accessors |
| **Target phase** | Implemented at **P5**; the P2/P3 surface port and deployment belong to their owning phases (ADR-016 §4: P11) |
| **Compatibility requirement** | Both backends keep the SAME product semantics; the gate's equivalence check compares live catalog against live catalog |
| **Deletion condition** | Not deletable — no permanent second persistence abstraction may exist beside it |
| **Evidence required before deletion** | n/a |

## S4i — Phase-6 entity layer: the Work Item and recorded human ownership *(canonical, not legacy)*

**Modules:** `work_item.py` · `migrations/phase6_work_items.py`

> ### **P6-U1 — what turns CLAUDE.md rule 13 from a sentence into a mechanism.** *"Every open
> operational obligation has one accountable human owner"* has been written down since P0. Until
> this unit it was written down. `owner_id NOT NULL` alone is a **non-empty string check** — it is
> satisfied by `'system'`, by `'the ops team'`, and by whatever the caller had in hand. So ownership
> is a **foreign key into a record of authority**: `work_items(tenant, owner_id) →
> tenant_humans(tenant, human_id)`, with the owner required ACTIVE at the moment work is assigned.
> An owner Neyma never recorded is not a bad value; it is an **unspellable** one.

| | |
|---|---|
| **Current responsibility** | `tenant_humans` — the durable, attributed record of which named humans hold authority in a tenant (spec §7's two HUMAN roles). `work_items` — the unit of business responsibility and closure. `work_item.py` — machine **M1**: the fourteen §14 transitions as declarative data, their exact guards, GR-1 illegal handling, GR-3 OCC, GR-4 idempotency through P5's inbox, and K-1 `decision_ref` resolution |
| **### What is NOT here, deliberately** | **No user administration.** ADR-017 gives users-and-roles to the web control plane at **P11**; this is the *record*, not the admin surface. **No `rules` table** — K-1's `RULE` referent needs M12 and therefore **fails closed** rather than being stubbed open. **No `commit_key`** — a Work Item performs no external effect (entity point 38) and has nothing to reserve. **No new event transport, replay, audit, timer, persistence or idempotency machinery** — all five are P5's, used as-is |
| **Current execution authority** | ### **None. It ships dark.** M1's transitive import closure reaches no effect-capable module — asserted structurally, over every import spelling — and it mints zero witnesses and zero grants across all seven reachable states. ### **Its ONE importer is machine M2**, added at P6-CP-2 and named rather than absorbed: §5 of `02-pipeline-instance.machine.md` makes the attempt's owner *the Work Item's owner*, so M2 reads that owner from M1's roster instead of taking a string. M2 ships dark too, so the composite posture is unchanged — a guard asserts the ONLY importer is the P6 entity layer, and that no adapter, workflow, script or callback imports either |
| **### The mechanism, stated so it can be attacked** | Ownership moves on **exactly three** of the fourteen transitions (WI-1 assignment, WI-11 reassignment, WI-13 the reopening actor's assignee) and that is asserted over the table, not reviewed. `recorded_by_kind` is CHECK-constrained to `'human'`: if a model could record an authority, the model would be choosing who may own work. §7's *Automated detector* and *Agent / model* rows are structurally absent from the role CHECK, and `actor_type='model'` is refused on **every** M1 transition (C-6, GR-7). Offboarding **REFUSES** while the human owns non-terminal work and NAMES the items, so the ownerless state is unreachable through the supported path rather than detected after it |
| **### Guard failure and illegal transition are kept apart** | An omitted `(state, trigger)` pair raises, persists nothing, and records `IllegalTransitionAttempted` to the audit backbone **and** `security_events` — atomically with each other, in a commit of their own, because the transaction it is refusing is the one being abandoned. A **legal** trigger whose precondition is false emits **nothing at all**. Collapsing the two is expensive in both directions: every unmet guard becoming a security event teaches an operator to ignore security events |
| **Current risk** | Low, and bounded by having no caller. The residual risk is scope, not safety — Pipeline Instances and the other twelve machines are the next units |
| **Canonical destination** | This IS the canonical destination (spec §12.1, `entities/01-work-item.md`, `state-machines/01-work-item.machine.md`). It is also the **absorption target** S4 names: `workflow.py`'s responsibilities move here as the entity layer completes |
| **Disposition** | ### **KEEP** — canonical, forward-only. The ownership foreign key, the ACTIVE-owner requirement, the terminality trigger and the version-advance trigger may never be relaxed; `decision_ref` resolution may never become a non-null check; and `TRANSITIONS` must stay declarative data, because `AC-MACH-000`'s bijection with §14 is asserted against it |
| **Target phase** | Implemented at **P6** (U1); consumed by the Pipeline Instance and the remaining machines, then by **P7+** |
| **Compatibility requirement** | The aggregate type stays `work_item`; `accountable_owner_id` stays on **every** emitted event, so an audit reconstructs who was accountable from the beliefs of that day rather than from today's roster |
| **Deletion condition** | Not deletable — no permanent second unit-of-work authority may exist beside it |
| **Evidence required before deletion** | n/a |

## S4j — Phase-6 attempt layer: the Pipeline Instance *(canonical, not legacy)*

**Modules:** `pipeline_instance.py` · `migrations/phase6_pipeline_instances.py`

> ### **P6-CP-2 — what Neyma is actually DOING about an obligation, durably.** M1 answers *"what
> work exists, and who is accountable for it?"* This answers the next question a dispatcher asks:
> *"is the invoice for load 4471 going out, and can it go out twice?"* A Pipeline Instance is ONE
> durable attempt to produce ONE effect (`02-pipeline-instance.machine.md` §3: *the Pipeline
> Instance IS the command*), and the two properties it exists for are **one attempt at a time, one
> effect ever** and **never both, never neither**.

| | |
|---|---|
| **Current responsibility** | `pipeline_instances` — one attempt, tenant-first, holding the **Layer-1 reservation** `UNIQUE(tenant, commit_key) WHERE state NOT IN (terminal)`. `pipeline_instance.py` — machine **M2**: the twenty-five §14 transitions as declarative data, their exact guards, the PL-8 checkpoint co-commit, the PL-9 claim co-commit, GR-1 illegal handling, GR-3 OCC, GR-4 idempotency through P5's inbox, and GR-5/GR-6's refusal to turn "we do not know" into "it failed" |
| **### What is NOT here, deliberately** | **No second checkpoint and no second CAS** — PL-8 and PL-9 call P3's kernel through its locked entry points and put the pipeline's own write inside the kernel's transaction; a re-implementation would be the dual effect-authority system rule 17 forbids. **No grant STATE column** — SD-2 gives the effect ONE ledger row with EIGHT states and M3 owns it; this row REFLECTS it and points at it by foreign key. **No money** — the `exposure` an `OutcomeUnknown` carries stays on the canonical event; the row records the event id and the specific question. **No reopening** — §24: reopening is at the Work Item, a retry is a NEW instance, and undoing a verified effect is a Compensation (M10) |
| **Current execution authority** | ### **None. It ships dark.** M2's transitive import closure reaches no effect-capable module — asserted by walking the whole closure, not by reading the imports — and a guard asserts the package and `scripts/` contain **zero importers** of `pipeline_instance.py`. It mints witnesses and grants only when a caller hands it a `CheckpointKernel`, and the production `GateRegistry` population stays **EMPTY** until U8.1/P8 |
| **### The mechanism, stated so it can be attacked** | The reservation predicate IS §20's retry rule: the four terminal states are outside the index so a finished attempt frees the key, and `NEEDS_VERIFICATION` is **non-terminal** so an attempt whose outcome nobody can establish keeps holding it — "we cannot say whether it happened, therefore nobody may try again", enforced by a partial index rather than by a code path. `GRANTED` and everything downstream is unreachable without BOTH a real `checkpoint_witnesses` row and a real `effect_grants` row (two foreign keys plus a trigger). `FAILED` is unreachable without affirmative `failure_proof`, at the guard AND at the database. Terminal is **absolutely** final by trigger — there is no WI-13-style door — because retry-in-place on an attempt that may already have moved money is §15's named illegality |
| **Current risk** | Low, and bounded by having no caller. The residual risk is scope, not safety — M3–M13 and the remaining 95 transitions are the next units, and three named debts (P6-D11/D12/D13) recorded where §14 and the event registry leave a seam this unit refused to improvise across. ### **P6-D11 IS SINCE RESOLVED** ([record](p6-d11-f2-ordering-contract-record.md)): the F2 ordering contract is now explicit — strict means ORDER and never CONTIGUITY — and M2 declares `previous_aggregate_version` on every envelope it emits, through the one `_envelope()` factory it already used. **That one keyword argument is the only M2 change; P6-D12 and P6-D13 stay open** |
| **Canonical destination** | This IS the canonical destination (spec §12.2/§19, `entities/02-pipeline-instance.md`, `state-machines/02-pipeline-instance.machine.md`). It is the **absorption target** S4 names for effect dispatch: no effect may be dispatched outside a Pipeline Instance once the machines are live |
| **Disposition** | ### **KEEP** — canonical, forward-only. The Layer-1 reservation index may never lose its UNIQUE or widen its predicate; the witness/grant foreign keys and the post-checkpoint trigger may never be relaxed; `failure_proof` may never become optional; PL-8 and PL-9 may never stop sharing a transaction with the kernel's own writes; and `TRANSITIONS` must stay declarative data, because `AC-MACH-000`'s bijection with §14 is asserted against it |
| **Target phase** | Implemented at **P6** (CP-2); completed by M3–M13, then consumed by **P7+** and by P12's supervised writes |
| **Compatibility requirement** | The aggregate type stays `pipeline_instance` (F2, STRICT per-aggregate ordering); `work_item_id` and `accountable_owner_id` stay on **every** emitted event, so an audit reconstructs what was owed and who owed it from the beliefs of that day |
| **Deletion condition** | Not deletable — no permanent second effect-dispatch authority may exist beside it |
| **Evidence required before deletion** | n/a |
...
| ID | Finding | Why nonblocking |
|---|---|---|
| **P6-D1** | K-1 names six human-decision event types; **`HumanResolved` is not among the 118 canonical contracts.** `ExceptionResolved` (M9, EC-3/EC-6) is the canonical name for what looks like the same fact | The resolvable set is K-1's names ∩ the registry (five), asserted by NAME. Substituting one for the other would be this unit amending K-1; **M9 owns that determination**. A `decision_ref` naming an `ExceptionResolved` refuses today — the safe direction |
| **P6-D2** | `CorrectionInvalidatedAnEffect` is listed in M1 §33 "Events consumed" but has **no §14 row**; §27 routes it out of band to M10, and entity point 32 says it *"may raise a Compensation-bearing obligation"* — i.e. it creates a NEW Work Item rather than transitioning one | It is therefore excluded from M1's trigger set. Including it would make every legitimate delivery an `IllegalTransitionAttempted` — a **false** security event, strictly worse than a missing one. Closes when M10 lands |
| **P6-D3** | `offboard_human` makes the ownerless state unreachable through the supported API, but a row edited around it by an operator with a connection could still retire an owner | The detector `WorkItemMachine.ownerless()` **sees** it (proven with a positive control that breaks the row directly). The Sev-0 raise and the "no consequential Pipeline may start" half are **M9 and M2** — later P6 units, not inventable here |
| **P6-D4** | K-1's `RULE` referent cannot resolve: there is no `rules` table until M12 | **Fails closed** with a named reason. A stub accepting any rule id would make *"closed by an active rule"* true of rules that do not exist |
| **P6-D5** | `WI-3`'s `obligation_satisfied` is a fact the CALLER states; M1 will not derive it from a `PipelineClosed` arriving | This is acceptance (d), not a gap: *billed is not paid*, and a Work Item is 1:N to its Pipelines. A machine that derived it would close obligations early and be right most of the time — the worst possible failure profile for money |
...
`scripts/migrate_phase2_tenant_first.py` (the canonical P2 migration CLI — forward-only, same standing as `migrations/`) · `scripts/migrate_phase3_checkpoint.py` (the canonical P3 checkpoint-schema migration CLI — create-only, idempotent, refuses non-canonical inputs; same standing) · `scripts/migrate_phase5_event_transport.py` (the canonical P5 event-transport migration CLI — create-only, idempotent, drops nothing, refuses a database that is not already Phase-2/Phase-3 canonical; same standing. It exists because `WorkflowStore` builds only an EMPTY database canonical and otherwise touches nothing, so extending a live schema stays an explicit act with a human behind it) · `scripts/generate_event_contracts.py` (the P5 U5.3 canonical-contract derivation — parses `events/registry.md` §3/§5/§8 and the F1–F14 family files and emits `src/freight_recon/event_contracts_data.json`; `--check` reports staleness without writing. It exists so the runtime contracts are the specification's mechanical projection rather than a second hand-maintained list, and a guard re-derives it on every suite run so a spec edit without a regeneration fails the build. Never imported by runtime) · `scripts/check_env.py` (the fail-fast Python-floor check; runs before any install) · `scripts/mutate_phase3_guards.py` (the P3 mutation battery — **both** the guard battery M1–M8 and the kernel battery K1–K11 added by the P3 findings remediation, which mutates `checkpoint.py`, `workflow.py`, the P3 migration and the recorded rebaseline anchor; evidence infrastructure, never imported by runtime; holds originals **in memory**, purges `__pycache__` around every mutation and asserts byte-for-byte restoration, so it never uses `git checkout`/`restore`/`stash`/`clean`; supports programmatic mutators for changes a find/replace cannot express, such as the canonical step-6/7 swap; extend it rather than adding a second mutation route) · `scripts/mutate_phase4_boundary.py` (the P4 boundary mutation battery — mutates `effect_boundary.py` and the import gate in `import_probe.py`, plus a CREATE mutator that resurrects a deleted effect path to prove the deletion is guarded; same in-memory save/restore, `__pycache__`-purge and byte-for-byte restoration doctrine; evidence, not adjudication) · `scripts/mutate_phase5_contracts.py` (the P5 U5.3 contract mutation battery — 37 cases: one removing each refusal in `event_contracts.py`, the outbox and inbox contract gates, two that edit the **canonical specification itself** to prove the anti-drift guard is real, and seven that mutate the PARSER and regenerate the data — the coverage hole an independent review named, since a parser defect is inert until the data is re-derived. Same in-memory save/restore, `__pycache__`-purge and byte-for-byte restoration doctrine as the P3 and P4 batteries; evidence, never adjudication) · `scripts/build_gc1_corpus.py` (the P5 U5.4 golden-corpus builder — derives `eval/fixtures/gc1-corpus.json` and its pinned digests from real canonical contracts, validating every event as a PRODUCER would so the fixture cannot contain a fact the runtime would refuse to emit; `--check` reports staleness without writing, and is what CI runs. ### Running it WITHOUT `--check` re-pins the digest, which the acceptance oracle says a human must explain — so it is never automatic and never invoked by a test. Never imported by runtime) · `scripts/mutate_phase5_replay.py` (the P5 U5.4+U5.5+U5.6 replay/audit mutation battery — 18 cases, of which R1/R2 are the registry's MANDATED proof that replay cannot call an adapter (each adds an effect-capable import and confirms the import-closure guard goes red), and R3/R4/R11 reintroduce defects this build actually shipped: an arrival-ordered fold, a double-counted redelivery, and an audit that flattened facts across aggregates. Builder cases regenerate GC-1 because a builder mutation is inert until the fixture is rebuilt; the corpus and its pins are restored byte-for-byte. Same in-memory save/restore and `__pycache__`-purge doctrine; evidence, never adjudication) · `scripts/mutate_phase6_work_item.py` (the P6-U1 Work Item / ownership mutation battery — 27 cases against `work_item.py`, `migrations/phase6_work_items.py` and `schema.py`. W1–W8 turn each ownership mechanism back into a convention (the roster lookup, the ACTIVE check, the offboarding refusal, the non-human-identity CHECK, the human-only `recorded_by_kind`, the ownership foreign key) and show which guard notices; W3 reintroduces a defect this unit ACTUALLY SHIPPED — the `IllegalTransitionAttempted` record written inside the transaction it was abandoning, so the refusal worked and `security_events` was empty. ### Two cases were first reported MISS and both reports were CORRECT about the PROBE rather than the code: W18 used a state-CHANGING interleaving the UPDATE's other predicate caught on its own, re-aimed at a WI-4 self-loop where only the version predicate can see it; W16's trigger looked redundant with the table CHECK until the BLANK-string case the CHECK cannot see was asserted. Neither guard was weakened to make a mutant pass. Same in-memory save/restore, `__pycache__`-purge and byte-for-byte restoration doctrine as the P3/P4/P5 batteries; evidence, never adjudication) · `scripts/mutate_phase6_pipeline_instance.py` (the P6-CP-2 Pipeline Instance mutation battery — 40 cases against `pipeline_instance.py`, `migrations/phase6_pipeline_instances.py`, `checkpoint.py`, `migrations/phase5_event_transport.py` and `schema.py`. Each turns one attempt-safety mechanism back into a convention and shows which guard notices: the Layer-1 reservation losing its UNIQUE or widening its predicate, `GRANTED` reachable without a witness, terminal stops being terminal, `FAILED` without a proof at the guard AND at the database, a timer moving `NEEDS_VERIFICATION`, PL-8/PL-9 leaving the kernel's transaction, and the claim CAS dropping its brake revalidation. P1b, P6b and P8b reintroduce defects this unit ACTUALLY SHIPPED — an absorption that did not advance the holder's version and was therefore uninsertable on a strict aggregate, the strict-ordering trigger refusing the F14 security record GR-1 requires, and a consequential event pinning a fresh read instead of the witness. ### Eight cases were first reported MISS and every report was CORRECT about the PROBE rather than the code: the co-commit mutants needed fault injection (a stale row snapshot) rather than a brake, because a brake refuses on both sides of the boundary; the owner and second-ledger mutants needed the INSERT-side and the negative-case guards that did not yet exist. Four new guards were ADDED and no guard was weakened to make a mutant pass. Same in-memory save/restore, `__pycache__`-purge and byte-for-byte restoration doctrine as the P3/P4/P5/M1 batteries; evidence, never adjudication) · `scripts/mutate_p6_d11_stream_link.py` (the P6-D11 STRICT-ORDER stream-link mutation battery — 15 cases against `event_inbox.py`, `event_outbox.py`, `event_envelope.py`, `pipeline_instance.py` and the canonical `events/registry.md` itself. `M1` restores the EXACT defect the unit closed — the pre-fix contiguity rule — and the battery would be worthless without it. ### Most of it attacks the SAFETY half, because a fix that merely stopped parking would be a larger and quieter bug than the one it closed: `M2` removes the block so a genuinely lost event no longer parks, `M3` inverts the comparison so the happy path stalls, `M4` lets a reordered delivery through, `M6`/`M6b`/`M6c`/`M6d` forge the link four ways (unverified, aggregate-blind, tenant-blind, and self-referential), `M8` reads an absent link as "there is nothing before me", `M9` adds a second envelope construction site in M2 to prove the AST single-site guard is load-bearing, and `M10` deletes the §8 clarification from the specification. ### `M6d` was first reported MISS and the report was CORRECT about the PROBE: the mutant genuinely misbehaves — it breaks 22 nodes — but only where an event rides at an ALREADY-EMITTED version, which is `IllegalTransitionAttempted`'s position and not the chain case it was first aimed at. The guard was re-aimed and STRENGTHENED to cover an attack at both an occupied and a silent version; no guard was weakened to make a mutant pass. Same in-memory save/restore, `__pycache__`-purge and byte-for-byte restoration doctrine as the P3/P4/P5/M1/M2 batteries; evidence, never adjudication) · `scripts/postgres_p5_gate.py` (the P5 PostgreSQL gate — EXECUTES against a real server: applies the migration, proves replaying it is a genuine no-op, proves tenant-first keys on the live catalog, proves all eight invariants REFUSE with a positive control so a read-only table cannot score full marks, proves the SQLite and PostgreSQL catalogs agree column-for-column, and drives the ACTUAL runtime classes through 13 probes including eight concurrent emitters on real threads. Writes a receipt; never invoked by the canonical suite, because requiring a database server would change what clean-clone reproducibility means) · `scripts/report_legacy_commit_identities.py` (read-only Phase-1 evidence probe)
