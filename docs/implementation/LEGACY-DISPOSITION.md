...
| **Current risk** | Low, and bounded by having no caller: nothing in the repository emits or consumes a canonical event yet. The residual risk is scope, not safety — the 105 contracts (U5.3), the GC-1 digest (U5.4), replay isolation (U5.5) and ADR-016's PostgreSQL store are all still absent, so this runs on SQLite only |
