...
**Still present — DEFERRED (it keeps R-07 OPEN):** `scripts/run_action_callback_server.py` (EP-1, **ADAPT** → pipeline client; its cdp_actuator import feeds the OperationRouter→OperatorAgent autonomous browser WRITE — the live R-07 write — a P12-scale integration). This was NOT deferred blind (CLAUDE.md §9).
