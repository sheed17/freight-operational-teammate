...
| **Current risk** | R-07 remains OPEN — the boundary is built and green but **four** effect-capable import edges (EP-1, EP-3, EP-8, EP-14) are not yet cut, so ungated paths still exist beside it |
...
**Still present — DEFERRED to a subsequent session (they keep R-07 OPEN):** `scripts/run_action_callback_server.py` (EP-1, **ADAPT** → pipeline client; its cdp_actuator import feeds the OperationRouter→OperatorAgent autonomous browser WRITE — the live R-07 write — a P12-scale integration) · `scripts/propose_ar_from_tms.py` (EP-3, **ADAPT** → pipeline client; reads + one nav-click, needs the F2 read-only CDP surface) · `scripts/orient_tms.py` (EP-8, **MAKE_READ_ONLY** — needs a read-only navigator; not a clean import removal) · `scripts/read_tms_browser_use.py` (EP-14, **MAKE_READ_ONLY** — the clean split is blocked because its `BrowserUseWriteLedger` is a TESTED capability with a P12 boundary-routed future, so it may not be deleted, and moving it would add an adapter-import edge the shrinking-only allowlist forbids). All four are browser-untestable in this environment and were NOT done blind (CLAUDE.md §9).
