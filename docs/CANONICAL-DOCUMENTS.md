...
| `implementation/u5-persistence-and-timers-evidence.md` | P5's durable timers (M-36 — a timeout is a durable timer, never a sweep) and its production PostgreSQL persistence, runtime included: the certified outbox/inbox/timer classes driven against a real server through `persistence.py`. Records the defect that would have corrupted production — `BEGIN IMMEDIATE` has no PostgreSQL equivalent, and with the advisory lock removed eight concurrent emitters allocated one sequence SIX times raising zero errors — plus the scalar-vs-aggregate `MAX`, the upsert qualification rule, and `sqlite3.Row`'s dual accessors. Explains why the PostgreSQL gate is a script rather than a skippable pytest node. True as of its own commit; **scores nothing** | **HISTORICAL (evidence)** |
| `implementation/POSTGRES-P5-GATE.json` | The PostgreSQL gate receipt: 9 schema invariants + 13 runtime probes executed against a real server, with an explicit `_what_this_does_NOT_prove` field | **HISTORICAL (evidence)** |
| `implementation/p6-u1-work-item-ownership-implementation-record.md` | `P6-CP-1-CANDIDATE`'s implementer evidence: machine **M1**, the Work Item, and ownership as a RECORDED AUTHORITY rather than a string — why `owner_id NOT NULL` alone is a non-empty string check, and why the owner is therefore a FOREIGN KEY into `tenant_humans` whose role CHECK structurally excludes §7's detector and model rows. The fourteen §14 transitions as declarative data, with `AC-MACH-000`'s bijection asserted by EXACT SET EQUALITY plus a positive control proving the oracle is a set comparison and not a count; why a guard failure and an illegal transition are deliberately kept apart; and five defects this build found in its own work — the `IllegalTransitionAttempted` record that was rolled back with the refusal it records (leaving `security_events` empty after a hostile attempt); a parked trigger that looped forever; ### the pre-commit suite that ran GREEN only because the new guard module was still UNTRACKED, which is the SAME false green P5's independent review rejected a candidate for and which the finalizer refused before writing any status; and two mutants whose MISS reports were correct about the PROBE rather than the code. Records the dark posture as MEASURED, and names in full what the candidate owes and what P6 still owes (12 machines, 120 transitions). ### It is the IMPLEMENTER's record: true as of its own commit, **scores nothing**, and is **NOT an independent review** | **HISTORICAL (evidence)** |
| `implementation/p6-cp2-pipeline-instance-implementation-record.md` | `P6-CP-2-CANDIDATE`'s implementer evidence: machine **M2**, the Pipeline Instance — one durable ATTEMPT to produce one effect, and the two properties it exists for. **One attempt at a time, one effect ever**: PL-1's Layer-1 reservation `UNIQUE(tenant, commit_key) WHERE state NOT IN (terminal)`, why a second proposal is ABSORBED rather than raced, and why the predicate of that index IS §20's retry rule — `NEEDS_VERIFICATION` is non-terminal, so it keeps holding the key and nothing may retry an effect nobody can say happened. **Never both, never neither**: PL-8 and PL-9 call P3's kernel through ADDITIVE locked entry points so the witness insert, the grant mint, the claim CAS and the pipeline's own writes share ONE transaction, with the CAS's six WHERE-clause predicates untouched. The twenty-five §14 transitions as declarative data, `AC-MACH-000`'s bijection by EXACT SET EQUALITY with a same-count-substitution positive control, and the exhaustive 377-pair illegal sweep over a population proven by reaching all sixteen states. ### Records FIVE material defects this build found in its own work, and THREE canonical seams it refused to improvise across: `IllegalTransitionAttempted` was UNINSERTABLE on a strict-order aggregate, so GR-1's refusal worked while the evidence of it could not be written — corrected toward `events/registry.md` §8, which classifies ordering BY FAMILY and names F14 order-tolerant; PL-1b's absorption was uninsertable for the same reason, which is how §14's *attach evidence* was proven to mean a durable write; a stale trigger TEXT read as a present trigger; the claim-CAS structural guard had been anchored on a function NAME and stopped covering anything; and eight mutants whose MISS reports were correct about the PROBE rather than the code, answered with four NEW guards and no weakened one. Discloses in full what this session could NOT reproduce — the `socket.bind`-blocked callback tests and the TLS-blocked clean-clone gate — and names what the candidate owes. ### It is the IMPLEMENTER's record: true as of its own commit, **scores nothing**, and is **NOT an independent review** | **HISTORICAL (evidence)** |
| `implementation/p6-d11-f2-ordering-contract-record.md` | ### **The `P6-D11` resolution: the STRICT-ORDER F2 ordering contract, made explicit.** `events/registry.md` §8 required strict per-aggregate ORDER and the dedup inbox implemented CONTIGUITY — a rule §8 never stated and could not state, because `GR-2` is discharged by a **co-commit**, so a transition whose canonical event belongs to another machine's aggregate advances its own version and emits nothing on its own stream. **Nine** of M2's transitions do that (the eight `CONSUMES` rows plus `PL-11d`), so `CheckpointPassed` rode at a version the consumer could never reach: M3 would have parked at the first hole and minted **zero** Effect Grants, on every load, forever — silently, with no failure anyone could see. ### Records the decision (**the successor declares `previous_aggregate_version`; a consumer blocks on an UNAPPLIED PREDECESSOR, never on an absent version**), the three alternatives weighed against it and why each was rejected on runtime correctness rather than on effort, and the evidence that the park which MUST still happen still happens — a lost or reordered event parks exactly as before. Adds **no** canonical event, moves **no** transition, migrates **no** schema, and leaves every historical event byte-identical. Records `P6-D24`/`P6-D25`/`P6-D26` rather than actioning them. ### It is the IMPLEMENTER's record: **scores nothing**, is **NOT an independent review**, and does **NOT** unblock `M3` — it owes its own fresh independent review, a separate adjudication and one canonical finalizer | **HISTORICAL (evidence)** |
...
| `implementation/p5-final-adjudication-report-91ba4e6.md` | ### **The SEPARATE FINAL ADJUDICATION of P5** content commit `91ba4e6`, by a session that neither implemented, remediated, reviewed nor finalized it. Verdict **P5 ACCEPTED**; its §E is the verbatim source of the fourteen weighted results now in the registry, and its §H is the mechanical determination — from the P4 and R-07 closure topologies — that the adjudicating session **may not run the finalizer**. It reproduced the suite, both gates, both mutation batteries and its own import-closure probe rather than reading receipts, and reported its own first probe as defective. `.sha256` sidecar alongside; preserved at `refs/preserve/p5-final-adjudication-91ba4e6`. ### **It adjudicated `91ba4e6` — NOT the closure commit that carries it**, which owes its own targeted review and targeted adjudication | **HISTORICAL (evidence)** |
...
| `implementation/p6-cp1-targeted-adjudication-report-ca8c070.md` | ### **The SEPARATE TARGETED ADJUDICATION of P6 checkpoint `P6-CP-1`** at candidate content commit `ca8c070` (tree `29ad9c25e`), by a third session that neither implemented nor remediated machine M1 and did not perform its independent review. Verdict **ADJUDICATE PASS — P6-CP-1 READY FOR FINALIZER**. It reproduced the 41/41 mutation battery, P5's replay (24/24) and contract (37/37) batteries, and the `TEST-NODE-MANIFEST` set difference by exact identity, and it found one of its **own** probes not capable of failing and corrected it before believing it. ### **Its §11 records the nonblocking residuals A-01/A-02/A-03 and its stated limitation — the canonical suite and clean-clone gate were NOT fully reproduced in its sandbox** (no listening sockets, blocked pypi TLS); the finalizer that landed the checkpoint re-ran both in full. `.sha256` sidecar alongside; preserved at `refs/preserve/p6-cp1-targeted-adjudication-ca8c070`. ### **It adjudicated a CHECKPOINT, not the phase:** it scored no P6 acceptance criterion and marked nothing COMPLETE | **HISTORICAL (evidence)** |
| `implementation/p6-cp2-targeted-readjudication-report-1aaf943.md` | ### **The SEPARATE TARGETED RE-ADJUDICATION of P6 checkpoint `P6-CP-2`** (machine M2, the Pipeline Instance) at candidate content commit `1aaf943` (tree `1d859547798c`), by a session that neither implemented, remediated, nor independently reviewed the candidate, and did not author the prior pre-review adjudication it supersedes. Verdict **ACCEPT — M2 MAY PROCEED TOWARD LANDING**; no blocking defect survives. It re-derived F-01 and F-02 closure itself, ruled on all seven review findings, **OVERTURNED IN PART** the review's R-3 by mutation (the landing-gate existence check does exist, in `test_roadmap_completeness_control.py`'s `test_a_claimed_independent_checkpoint_review_must_cite_a_report_that_exists`), and **UPHELD R-4** — that a targeted adjudication created BEFORE any independent review existed is not the route's third step — which is why this artifact is a *re*-adjudication. ### **IT SUPERSEDES `refs/preserve/p6-cp2-targeted-adjudication-1aaf943` (verdict BLOCKED ON EVIDENCE) as the adjudication of record; that prior adjudication is RETAINED, not discarded** — its N-01 preservation act stands at `refs/preserve/p6-cp2-product-driver-bypass-evidence-1aaf943`. ### **Its §5 records the nonblocking findings A-01/A-02 and its stated limitation** — the canonical suite and the clean-clone gate were NOT reproduced in its sandbox (no listening sockets, intercepted pypi TLS); the finalizer, on a capable host, is what discharges both. Preserved at `refs/preserve/p6-cp2-targeted-readjudication-1aaf943` (commit `6f87a1a`, parent `1aaf943`). ### **It adjudicated a CHECKPOINT, not the phase:** it scored no P6 acceptance criterion and marked nothing COMPLETE | **HISTORICAL (evidence)** |
| `implementation/p6-d11-independent-review-report-021a9a2.md` | ### **The FRESH TARGETED INDEPENDENT REVIEW of the `P6-D11` strict-order F2 ordering resolution** at content commit `021a9a2816f5` (tree `f1df545d4fdb`), by a session that neither implemented nor remediated it and had no part in any prior P6 unit. Verdict **ACCEPT FOR SEPARATE TARGETED ADJUDICATION**. It re-derived the root cause by driving the real `PipelineMachine` to `CLAIMED` and reading the real outbox rather than reading the record, confirmed the **nine** silent transitions mechanically (the eight `CONSUMES` rows plus `PL-11d`, with `PL-15x` correctly excluded), upheld the predecessor-link branch against A/B/C with each premise independently checked, and returned five findings **none of which blocks**: **F-1** the status authority telling a fresh session to run a `P6-CP-2` finalizer that had already run; **F-2** the link is enforced only where declared; **F-3** (new) a strict-order consumer that family-filters `IllegalTransitionAttempted` out of the `pipeline_instance` stream parks forever — now answered by `events/registry.md` §8's complete-stream rule; **F-4/F-5** observations. It also determined independently, from `SUITE-RESULT.json` and `GATE-RESULT.json`, that **`c2def38` already fully finalized `P6-CP-2`**. ### **IT WAS LOST AND RECOVERED:** no preservation ref existed, the reviewer's session scratchpad was cleared, and the adjudicator refused to substitute its own verification for it; the landing recovered the bytes byte-exactly from the reviewing session's own durable tool-invocation record. Preserved at `refs/preserve/p6-d11-independent-review-021a9a2` (commit `317fe12f`, parent `021a9a2`), with a `.sha256` sidecar pinning the artifact **as received** — the body beneath the disarming banner. ### **It is a REVIEW, not an adjudication:** it scored no criterion, marked nothing COMPLETE and authorized no finalization | **HISTORICAL (evidence)** |
| `implementation/p6-d11-targeted-adjudication-report-021a9a2.md` | ### **The SEPARATE TARGETED ADJUDICATION of the `P6-D11` resolution** at content commit `021a9a2816f5` (tree `f1df545d4fdb`), by a third session that neither implemented, remediated nor independently reviewed the candidate. Verdict **ACCEPT** — no material correctness defect survives and **no product-code change is owed**. ### **IT ADJUDICATED WITHOUT THE REVIEW REPORT AND SAID SO** rather than pretending otherwise: the report was "nowhere on disk, in the repo, or on any `refs/preserve` ref", so it declined to summarise it, re-derived every material claim itself, and made preservation hard landing condition **A-1** — explicitly refusing to let its own verification stand in for the missing review. It upheld the root cause, the predecessor-link contract across rollback, concurrency, duplicate, reorder, restart, replay, tenant/aggregate isolation, malformed, forged and cyclic inputs, the **nine**-transition count (its own first naive predicate returned ten), and `P6-D24` as a nonblocking M3 obligation; ruled the F14 family-filtering stall an **M3 authoring constraint that must be WRITTEN INTO §8**; established from receipts that **`c2def38` fully finalized `P6-CP-2` with no further finalizer owed**; and opened **`P6-D27`**, the chain-fork gap, nonblocking because unreachable through every canonical producer. Its three landing conditions (**A-1** preserve the review, **A-2** correct the stale finalizer prose, **A-3** the complete-stream rule) are discharged by the `P6-D11` landing commit. Preserved at `refs/preserve/p6-d11-targeted-adjudication-021a9a2` (commit `c164cc81`, parent `021a9a2`), with a validated `.sha256` sidecar. ### **It adjudicated a CONTENT CHANGE, not the phase:** it scored no P6 acceptance criterion, marked nothing COMPLETE, ran no finalizer and did not begin M3 | **HISTORICAL (evidence)** |
